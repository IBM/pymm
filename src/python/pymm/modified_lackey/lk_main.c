...
static Bool handle_client_request(ThreadId vg_tid, UWord* argv, UWord* ret)
{
   switch (argv[0])
   {
   case 0x1202: { //VG_USERREQ__GDB_MONITOR_COMMAND: {
      /* we use the gdb monitor user-request to dynamically 
         activate tracing */
      if(VG_(strcmp)("trace:on", (HChar*) argv[1]) == 0) {
         trace_active = True;
      }
      else if(VG_(strcmp)("trace:off", (HChar*) argv[1]) == 0) {
         trace_active = False;
      }
      break;
   }
