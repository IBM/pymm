...
protected:
	...
	{
		return "/mnt/pmem0/test-" + store_map::impl->name + store_map::numa_zone() + ".pool";
	}
 ...
 */
  ...
  void * old_root;
  {
		auto ccaptr = new ccpm::cca(&pe, v);
    void * root = ccaptr->allocate_root(sizeof(cc_vector));
    old_root = root;
    PLOG("root=%p", root);
    auto ccv = new (root) cc_vector(&pe, *ccaptr);

    ccv->container->push_back(1.1);
    ccv->container->push_back(2.2);
    ccv->container->push_back(3.3);
		
		ccv->commit();
    delete ccaptr;
	}

  /* simulated reconstitution */
  {
    auto ccaptr = new ccpm::cca(&pe, v, ccpm::accept_all);
    void *root = ::base(ccaptr->get_root());
    PLOG("root=%p", root);
    ASSERT_EQ(root, old_root);
    auto ccv = new (root) cc_vector(std::move(*static_cast<cc_vector *>(root)));
    ccv->rollback();

    /* let's modify an entry */
    auto& element = (double&) ccv->container->at(1);
    element += 10.0;
    ccv->add(&element, sizeof(double));
