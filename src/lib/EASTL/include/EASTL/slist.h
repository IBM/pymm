...
namespace eastl
{
	...
	struct SListNodeBase
	{
		...
		struct SListNode : public SListNodeBaseProxy< SListNode<T, R> >
		{
			T mValue;
		};
	...
	#else
		template <typename T, typename R>
		struct SListNode : public SListNodeBase<R>
		{
			T mValue;
		};
	#endif
	...
	/// SListIterator
	///
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	struct SListIterator
	{
		typedef SListIterator<T, Pointer, Reference, Tracker> this_type;
		typedef SListIterator<T, T*, T&, Tracker>      iterator;
		typedef SListIterator<T, const T*, const T&, Tracker> const_iterator;
		typedef eastl_size_t                           size_type;     // See config.h for the definition of eastl_size_t, which defaults to size_t.
		typedef ptrdiff_t                              difference_type;
		typedef T                                      value_type;
		typedef SListNode<T, Tracker>                  node_type;
	...
	public:
		SListIterator();
		SListIterator(const SListNodeBase<Tracker>* pNode);
	...
	public:
		...
		using tracker_type = typename Allocator::tracker_type;
		typedef SListNode<T, tracker_type>             node_type;
		typedef eastl_size_t                           size_type;     // See config.h for the definition of eastl_size_t, which defaults to size_t.
		typedef ptrdiff_t                              difference_type;
		#if EASTL_LIST_PROXY_ENABLED
			typedef SListNodeBaseProxy< SListNode<T, tracker_type> > base_node_type;
		#else
			typedef SListNodeBase<tracker_type> base_node_type; // We use SListNodeBase instead of SListNode<T, tracker_type> because we don't want to create a T.
		#endif
	...
	protected:
	   ...
	   ~SListBase();
		...
		SListNodeBase<tracker_type>* DoEraseAfter(SListNodeBase<tracker_type>* pNode);
		SListNodeBase<tracker_type>* DoEraseAfter(SListNodeBase<tracker_type>* pNode, SListNodeBase<tracker_type>* pNodeLast);
	...
	public:
		...
		typedef SListIterator<T, T*, T&, tracker_type>  iterator;
		typedef SListIterator<T, const T*, const T&, tracker_type> const_iterator;
	...
	protected:
		...
		template <typename InputIterator>
		node_type* DoInsertAfter(SListNodeBase<tracker_type>* pNode, InputIterator first, InputIterator last);

		template <typename Integer>
		node_type* DoInsertAfter(SListNodeBase<tracker_type>* pNode, Integer n, Integer value, true_type);

		template <typename InputIterator>
		node_type* DoInsertAfter(SListNodeBase<tracker_type>* pNode, InputIterator first, InputIterator last, false_type);

		node_type* DoInsertValueAfter(SListNodeBase<tracker_type>* pNode);
		node_type* DoInsertValuesAfter(SListNodeBase<tracker_type>* pNode, size_type n, const value_type& value);

		template<typename... Args>
		node_type* DoInsertValueAfter(SListNodeBase<tracker_type>* pNode, Args&&... args);
	...
	template <typename Tracker>
	inline SListNodeBase<Tracker>* SListNodeInsertAfter(SListNodeBase<Tracker>* pPrevNode, SListNodeBase<Tracker>* pNode)
	{
	...
	template <typename Tracker>
	inline SListNodeBase<Tracker>* SListNodeGetPrevious(SListNodeBase<Tracker>* pNodeBase, const SListNodeBase<Tracker>* pNode)
	{
	...
	template <typename Tracker>
	inline const SListNodeBase<Tracker>* SListNodeGetPrevious(const SListNodeBase<Tracker>* pNodeBase, const SListNodeBase<Tracker>* pNode)
	{
		while(pNodeBase && (pNodeBase->mpNext != pNode))
			pNodeBase = pNodeBase->mpNext;
		return pNodeBase;
	...
	template <typename Tracker>
	inline void SListNodeSpliceAfter(SListNodeBase<Tracker>* pNode, SListNodeBase<Tracker>* pNodeBeforeFirst, SListNodeBase<Tracker>* pNodeBeforeLast)
	{
		if((pNode != pNodeBeforeFirst) && (pNode != pNodeBeforeLast))
		{
			SListNodeBase<Tracker>* const pFirst    = pNodeBeforeFirst->mpNext;
			SListNodeBase<Tracker>* const pPosition = pNode->mpNext;
	...
	template <typename Tracker>
	inline void SListNodeSpliceAfter(SListNodeBase<Tracker>* pNode, SListNodeBase<Tracker>* pNodeBase)
	{
		SListNodeBase<Tracker>* const pNodeBeforeLast = SListNodeGetPrevious(pNodeBase, static_cast<SListNodeBase<Tracker>*>(nullptr));
	...
	template <typename Tracker>
	inline SListNodeBase<Tracker>* SListNodeReverse(SListNodeBase<Tracker>* pNode)
	{
		SListNodeBase<Tracker>* pNodeFirst = pNode;
		pNode = pNode->mpNext;
		pNodeFirst->mpNext = NULL;

		while(pNode)
		{
			SListNodeBase<Tracker>* const pTemp = pNode->mpNext;
	...
	template <typename Tracker>
	inline uint32_t SListNodeGetSize(SListNodeBase<Tracker>* pNode)
	{
		uint32_t n = 0;
		while(pNode)
		{
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	inline SListIterator<T, Pointer, Reference, Tracker>::SListIterator()
		: mpNode(NULL)
	{
		// Empty
	}
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	inline SListIterator<T, Pointer, Reference, Tracker>::SListIterator(const SListNodeBase<Tracker>* pNode)
		: mpNode(static_cast<node_type*>((SListNode<T, Tracker>*)const_cast<SListNodeBase<Tracker>*>(pNode))) // All this casting is in the name of making runtime debugging much easier on the user.
	{
		// Empty
	}
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	inline SListIterator<T, Pointer, Reference, Tracker>::SListIterator(const iterator& x)
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	inline typename SListIterator<T, Pointer, Reference, Tracker>::reference
	SListIterator<T, Pointer, Reference, Tracker>::operator*() const
	{
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	inline typename SListIterator<T, Pointer, Reference, Tracker>::pointer
	SListIterator<T, Pointer, Reference, Tracker>::operator->() const
	{
		return &mpNode->mValue;
	}
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	inline typename SListIterator<T, Pointer, Reference, Tracker>::this_type&
	SListIterator<T, Pointer, Reference, Tracker>::operator++()
	{
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	inline typename SListIterator<T, Pointer, Reference, Tracker>::this_type
	SListIterator<T, Pointer, Reference, Tracker>::operator++(int)
	{
		this_type temp(*this);
		mpNode = static_cast<node_type*>(&*mpNode->mpNext);
		return temp;
	...
	template <typename T, typename PointerA, typename ReferenceA, typename TrackerA, typename PointerB, typename ReferenceB, typename TrackerB>
	inline bool operator==(const SListIterator<T, PointerA, ReferenceA, TrackerA>& a,
						   const SListIterator<T, PointerB, ReferenceB, TrackerB>& b)
	{
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	inline bool operator!=(const SListIterator<T, Pointer, Reference, Tracker>& a,
						   const SListIterator<T, Pointer, Reference, Tracker>& b)
	{
		return a.mpNode != b.mpNode;
	}
	...
	inline SListBase<T, Allocator>::SListBase()
		: mNode(),
		  #if EASTL_SLIST_SIZE_CACHE
		  mSize(0, Destructible<tracker_type>()),
		  #endif
	...
	inline SListBase<T, Allocator>::~SListBase()
	{
		DoEraseAfter(static_cast<SListNodeBase<tracker_type>*>(&mNode), NULL);
	}
	...
	template <typename T, typename Allocator>
	inline auto SListBase<T, Allocator>::DoAllocateNode() -> SListNode<T, tracker_type>*
	{
	...
	template <typename T, typename Allocator>
	auto SListBase<T, Allocator>::DoEraseAfter(SListNodeBase<tracker_type>* pNode) -> SListNodeBase<tracker_type>*
	{
		node_type*     const pNodeNext     = static_cast<node_type*>((base_node_type*)pNode->mpNext);
		SListNodeBase<tracker_type>* const pNodeNextNext = (SListNodeBase<tracker_type>*)pNodeNext->mpNext;
	...
	template <typename T, typename Allocator>
	auto SListBase<T, Allocator>::DoEraseAfter(SListNodeBase<tracker_type>* pNode, SListNodeBase<tracker_type>* pNodeLast) -> SListNodeBase<tracker_type>*
	{
	...
	inline slist<T, Allocator>::slist(size_type n, const allocator_type& allocator)
		: base_type(allocator)
	{
		DoInsertValuesAfter((SListNodeBase<tracker_type>*)&mNode, n, value_type());
	}
	...
	inline slist<T, Allocator>::slist(size_type n, const value_type& value, const allocator_type& allocator)
		: base_type(allocator)
	{
		DoInsertValuesAfter((SListNodeBase<tracker_type>*)&mNode, n, value);
	}
	...
	{
		DoInsertAfter((SListNodeBase<tracker_type>*)&mNode, const_iterator((SListNodeBase<tracker_type>*)x.mNode.mpNext), const_iterator(NULL), false_type());
	}
	...
	inline slist<T, Allocator>::slist(std::initializer_list<value_type> ilist, const allocator_type& allocator)
		: base_type(allocator)
	{
		DoInsertAfter((SListNodeBase<tracker_type>*)&mNode, ilist.begin(), ilist.end());
	}
	...
	{
		DoInsertAfter((SListNodeBase<tracker_type>*)&mNode, first, last);
	}
	...
	{
		return iterator((SListNodeBase<tracker_type>*)mNode.mpNext.value());
	}
	...
	{
		return const_iterator((SListNodeBase<tracker_type>*)mNode.mpNext);
	}
	...
	{
		return iterator((SListNodeBase<tracker_type>*)&mNode);
	}
	...
	{
		return const_iterator((SListNodeBase<tracker_type>*)&mNode);
	}
	...
	{
		return iterator(SListNodeGetPrevious((SListNodeBase<tracker_type>*)&mNode, (SListNodeBase<tracker_type>*)position.mpNode));
	}
	...
	{
		return const_iterator(SListNodeGetPrevious((SListNodeBase<tracker_type>*)&mNode, (SListNodeBase<tracker_type>*)position.mpNode));
	}
	...
	{
		DoInsertValueAfter((SListNodeBase<tracker_type>*)&mNode, eastl::forward<Args>(args)...);
	}
	...
	{
		SListNodeInsertAfter((SListNodeBase<tracker_type>*)&mNode, (SListNodeBase<tracker_type>*)DoCreateNode(value));
		#if EASTL_SLIST_SIZE_CACHE
		   ++mSize;
		#endif
	}
	...
	{
		SListNodeInsertAfter((SListNodeBase<tracker_type>*)&mNode, (SListNodeBase<tracker_type>*)DoCreateNode());
		#if EASTL_SLIST_SIZE_CACHE
		   ++mSize;
		#endif
	...
	{
		return SListNodeGetSize((SListNodeBase<tracker_type>*)mNode.mpNext);
	}
	...
	{
		DoEraseAfter((SListNodeBase<tracker_type>*)&mNode, NULL);
	}
	...
	{
		SListNodeBase<tracker_type>* pNode = (SListNodeBase<tracker_type>*)&mNode;
	...
	{
		return iterator((SListNodeBase<tracker_type>*)DoInsertValueAfter(SListNodeGetPrevious((SListNodeBase<tracker_type>*)&mNode, (SListNodeBase<tracker_type>*)position.mpNode), value_type()));
	}
	...
	{
		return iterator((SListNodeBase<tracker_type>*)DoInsertValueAfter(SListNodeGetPrevious((SListNodeBase<tracker_type>*)&mNode, (SListNodeBase<tracker_type>*)position.mpNode), value));
	}
	...
	{
		// To do: get rid of DoAssignValues and put its implementation directly here.
		DoInsertValuesAfter(SListNodeGetPrevious((SListNodeBase<tracker_type>*)&mNode, (SListNodeBase<tracker_type>*)position.mpNode), n, value);
	}
	...
	{
		DoInsertAfter(SListNodeGetPrevious((SListNodeBase<tracker_type>*)&mNode, (SListNodeBase<tracker_type>*)position.mpNode), first, last);
	}
	...
	{
		return iterator((SListNodeBase<tracker_type>*)DoInsertValueAfter((SListNodeBase<tracker_type>*)position.mpNode, value));
	}
	...
	{
		return iterator((SListNodeBase<tracker_type>*)DoInsertValuesAfter((SListNodeBase<tracker_type>*)position.mpNode, n, value));
	}
	...
	{
		return iterator((SListNodeBase<tracker_type>*)DoInsertAfter((SListNodeBase<tracker_type>*)position.mpNode, ilist.begin(), ilist.end(), false_type()));
	}
	...
	{
		return iterator((SListNodeBase<tracker_type>*)DoInsertAfter((SListNodeBase<tracker_type>*)position.mpNode, first, last));
	}
	...
	{
		return iterator((SListNodeBase<tracker_type>*)DoInsertValueAfter(position.mpNode, eastl::forward<Args>(args)...));
	}
	...
	{
		return DoEraseAfter(SListNodeGetPrevious((SListNodeBase<tracker_type>*)&mNode, (SListNodeBase<tracker_type>*)position.mpNode));
	}
	...
	{
		return DoEraseAfter(SListNodeGetPrevious((SListNodeBase<tracker_type>*)&mNode, (SListNodeBase<tracker_type>*)first.mpNode), (SListNodeBase<tracker_type>*)last.mpNode);
	}
	...
	{
		return iterator(DoEraseAfter((SListNodeBase<tracker_type>*)position.mpNode));
	}
	...
	{
		return iterator(DoEraseAfter((SListNodeBase<tracker_type>*)before_first.mpNode, (SListNodeBase<tracker_type>*)last.mpNode));
	}
	...
	{ 
		...
		while(pNode && pNode->mpNext)
		{
			if(static_cast<node_type*>(&*pNode->mpNext)->mValue == value)
				DoEraseAfter((SListNodeBase<tracker_type>*)pNode); // This will take care of modifying pNode->mpNext.
			else
				pNode = pNode->mpNext;
		}
	}
	...
	inline void slist<T, Allocator>::splice(const_iterator position, this_type& x)
	{
		...
		if(x.mNode.mpNext) // If there is anything to splice...
		{
			...
			{
				SListNodeSpliceAfter(SListNodeGetPrevious((SListNodeBase<tracker_type>*)&mNode, (SListNodeBase<tracker_type>*)position.mpNode),
									 (SListNodeBase<tracker_type>*)&x.mNode,
									 SListNodeGetPrevious((SListNodeBase<tracker_type>*)&x.mNode, static_cast<SListNodeBase<tracker_type>*>(nullptr)));
	...
	inline void slist<T, Allocator>::splice(const_iterator position, this_type& x, const_iterator i)
	{
		...
		{
			SListNodeSpliceAfter(SListNodeGetPrevious((SListNodeBase<tracker_type>*)&mNode, (SListNodeBase<tracker_type>*)position.mpNode), 
								 SListNodeGetPrevious((SListNodeBase<tracker_type>*)&x.mNode, (SListNodeBase<tracker_type>*)i.mpNode), 
								 (SListNodeBase<tracker_type>*)i.mpNode);
	...
	inline void slist<T, Allocator>::splice(const_iterator position, this_type& x, const_iterator first, const_iterator last)
	{
		if(first != last) // If there is anything to splice...
		{
			if(mAllocator == x.mAllocator)
			{
				...
				SListNodeSpliceAfter(SListNodeGetPrevious((SListNodeBase<tracker_type>*)&mNode,       (SListNodeBase<tracker_type>*)position.mpNode),
									 SListNodeGetPrevious((SListNodeBase<tracker_type>*)&x.mNode,     (SListNodeBase<tracker_type>*)first.mpNode),
									 SListNodeGetPrevious((SListNodeBase<tracker_type>*)first.mpNode, (SListNodeBase<tracker_type>*)last.mpNode));
			}
			else
			{
	...
	inline void slist<T, Allocator>::splice_after(const_iterator position, this_type& x)
	{
		if(!x.empty()) // If there is anything to splice...
		{
			if(mAllocator == x.mAllocator)
			{
				SListNodeSpliceAfter((SListNodeBase<tracker_type>*)position.mpNode, (SListNodeBase<tracker_type>*)&x.mNode);
	...
	inline void slist<T, Allocator>::splice_after(const_iterator position, this_type& x, const_iterator i)
	{
		if(mAllocator == x.mAllocator)
		{
			SListNodeSpliceAfter((SListNodeBase<tracker_type>*)position.mpNode, (SListNodeBase<tracker_type>*)i.mpNode);
	...
	inline void slist<T, Allocator>::splice_after(const_iterator position, this_type& x, const_iterator first, const_iterator last)
	{
		if(first != last) // If there is anything to splice...
		{
			if(mAllocator == x.mAllocator)
			{
				...
				SListNodeSpliceAfter((SListNodeBase<tracker_type>*)position.mpNode, (SListNodeBase<tracker_type>*)first.mpNode, (SListNodeBase<tracker_type>*)last.mpNode);
			}
			else
			{
	...
	inline void slist<T, Allocator>::splice_after(const_iterator position, const_iterator before_first, const_iterator before_last)
	{
		if(before_first != before_last) // If there is anything to splice...
		{
				...
				iterator i((SListNodeBase<tracker_type>*)&mNode);
				iterator iEnd(NULL);
			...
			SListNodeSpliceAfter((SListNodeBase<tracker_type>*)position.mpNode, (SListNodeBase<tracker_type>*)before_first.mpNode, (SListNodeBase<tracker_type>*)before_last.mpNode);
		}
	}
	...
	inline void slist<T, Allocator>::splice_after(const_iterator position, const_iterator previous)
	{
		...
		SListNodeSpliceAfter((SListNodeBase<tracker_type>*)position.mpNode, (SListNodeBase<tracker_type>*)previous.mpNode, (SListNodeBase<tracker_type>*)previous.mpNode->mpNext);
	}
	...
	{
		if(mNode.mpNext)
			mNode.mpNext = static_cast<node_type*>((base_node_type*)SListNodeReverse((SListNodeBase<tracker_type>*)mNode.mpNext));
	}
	...
	void slist<T, Allocator>::DoAssign(InputIterator first, InputIterator last, false_type)
	{
		...
		if(first == last)
			DoEraseAfter((SListNodeBase<tracker_type>*)pNodePrev, NULL);
		else
			DoInsertAfter((SListNodeBase<tracker_type>*)pNodePrev, first, last);
	}
	...
	void slist<T, Allocator>::DoAssignValues(size_type n, const value_type& value)
	{
		...
		if(n)
			DoInsertValuesAfter((SListNodeBase<tracker_type>*)pNodePrev, n, value);
		else
			DoEraseAfter((SListNodeBase<tracker_type>*)pNodePrev, NULL);
	}
	...
	template <typename InputIterator>
	inline typename slist<T, Allocator>::node_type*
	slist<T, Allocator>::DoInsertAfter(SListNodeBase<tracker_type>* pNode, InputIterator first, InputIterator last)
	{
	...
	template <typename Integer>
	inline typename slist<T, Allocator>::node_type*
	slist<T, Allocator>::DoInsertAfter(SListNodeBase<tracker_type>* pNode, Integer n, Integer value, true_type)
	{
	...
	template <typename InputIterator>
	inline typename slist<T, Allocator>::node_type*
	slist<T, Allocator>::DoInsertAfter(SListNodeBase<tracker_type>* pNode, InputIterator first, InputIterator last, false_type)
	{
		for(; first != last; ++first)
		{
			pNode = SListNodeInsertAfter((SListNodeBase<tracker_type>*)pNode, (SListNodeBase<tracker_type>*)DoCreateNode(*first));
			#if EASTL_SLIST_SIZE_CACHE
				++mSize;
			#endif
		}
	...
	template <typename T, typename Allocator>
	inline typename slist<T, Allocator>::node_type*
	slist<T, Allocator>::DoInsertValueAfter(SListNodeBase<tracker_type>* pNode)
	{
		#if EASTL_SLIST_SIZE_CACHE
			pNode = SListNodeInsertAfter((SListNodeBase<tracker_type>*)pNode, (SListNodeBase<tracker_type>*)DoCreateNode());
			++mSize;
			return static_cast<node_type*>((base_node_type*)pNode);
		#else
			return static_cast<node_type*>((base_node_type*)SListNodeInsertAfter((SListNodeBase<tracker_type>*)pNode, (SListNodeBase<tracker_type>*)DoCreateNode()));
		#endif
	}
	...
	template<typename... Args>
	inline typename slist<T, Allocator>::node_type*
	slist<T, Allocator>::DoInsertValueAfter(SListNodeBase<tracker_type>* pNode, Args&&... args)
	{
		SListNodeBase<tracker_type>* pNodeNew = (SListNodeBase<tracker_type>*)DoCreateNode(eastl::forward<Args>(args)...);
	...
	template <typename T, typename Allocator>
	inline typename slist<T, Allocator>::node_type*
	slist<T, Allocator>::DoInsertValuesAfter(SListNodeBase<tracker_type>* pNode, size_type n, const value_type& value)
	{
		for(size_type i = 0; i < n; ++i)
		{
			pNode = SListNodeInsertAfter((SListNodeBase<tracker_type>*)pNode, (SListNodeBase<tracker_type>*)DoCreateNode(value));
			#if EASTL_SLIST_SIZE_CACHE
