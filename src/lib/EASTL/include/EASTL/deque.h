...
namespace eastl
{
	...
	///
	template <typename T, typename Pointer, typename Reference, typename Tracker, unsigned kDequeSubarraySize, bool kTracked>
	struct DequeIterator
	{
		typedef DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked> this_type;
		typedef DequeIterator<T, T*, T&, Tracker, kDequeSubarraySize, false>       iterator;
		typedef DequeIterator<T, T*, T&, Tracker, kDequeSubarraySize, true>        tracked_iterator;
		typedef DequeIterator<T, const T*, const T&, Tracker, kDequeSubarraySize, false> const_iterator;
	...
	public:
		...
		DequeIterator(const tracked_iterator& x);
	...
	protected:
		...
		template <typename, typename, typename, typename, unsigned, bool>
		...
		template <typename U, typename PointerA, typename ReferenceA, typename TrackerA, typename PointerB, typename ReferenceB, typename TrackerB, unsigned kDequeSubarraySizeU, bool kTrackedU>
		friend bool operator==(const DequeIterator<U, PointerA, ReferenceA, TrackerA, kDequeSubarraySizeU, kTrackedU>&,
							   const DequeIterator<U, PointerB, ReferenceB, TrackerB, kDequeSubarraySizeU, kTrackedU>&);

		template <typename U, typename PointerA, typename ReferenceA, typename TrackerA, typename PointerB, typename ReferenceB, typename TrackerB, unsigned kDequeSubarraySizeU, bool kTrackedA, bool kTrackedB>
		friend bool operator!=(const DequeIterator<U, PointerA, ReferenceA, TrackerA, kDequeSubarraySizeU, kTrackedA>&,
							   const DequeIterator<U, PointerB, ReferenceB, TrackerB, kDequeSubarraySizeU, kTrackedB>&);

		template <typename U, typename PointerU, typename ReferenceU, typename TrackerU, unsigned kDequeSubarraySizeU, bool kTrackedU>
		friend bool operator!=(const DequeIterator<U, PointerU, ReferenceU, TrackerU, kDequeSubarraySizeU, kTrackedU>& a,
							   const DequeIterator<U, PointerU, ReferenceU, TrackerU, kDequeSubarraySizeU, kTrackedU>& b);
		...
		template <typename U, typename PointerA, typename ReferenceA, typename TrackerA, typename PointerB, typename ReferenceB, typename TrackerB, unsigned kDequeSubarraySizeU, bool kTrackedA, bool kTrackedB>
		friend typename DequeIterator<U, PointerA, ReferenceA, TrackerA, kDequeSubarraySizeU, kTrackedA>::difference_type
		operator-(const DequeIterator<U, PointerA, ReferenceA, TrackerA, kDequeSubarraySizeU, kTrackedA>& a,
				  const DequeIterator<U, PointerB, ReferenceB, TrackerB, kDequeSubarraySizeU, kTrackedB>& b);

	protected:
/* Iterators are not normally persisted, so the are not generally tracked.
 ...
 */
		...
		this_type copy(Tracker &tr, const iterator& first, const iterator& last, true_type);  // true means that value_type has the type_trait has_trivial_relocate,
		this_type copy(Tracker &tr, const iterator& first, const iterator& last, false_type); // false means it does not.

		void copy_backward(Tracker &tr, const iterator& first, const iterator& last, true_type);  // true means that value_type has the type_trait has_trivial_relocate,
		void copy_backward(Tracker &tr, const iterator& first, const iterator& last, false_type); // false means it does not.
	...
	protected:
		typedef DequeIterator<T, T*, T&, tracker_type, kDequeSubarraySize, true>       internal_iterator;
	...
	public:
		...
		typedef ptrdiff_t                                                difference_type;
		typedef DequeIterator<T, T*, T&, tracker_type, kDequeSubarraySize, false>      iterator;
		typedef DequeIterator<T, const T*, const T&, tracker_type, kDequeSubarraySize, false> const_iterator;
	...
	private:
		typedef typename base_type::internal_iterator                    internal_iterator;
	public:
	...
	void DequeBase<T, Allocator, kDequeSubarraySize>::DoReallocPtrArray(size_type nAdditionalCapacity, Side allocationSide)
	{
		...
		auto tr = mem_wrap(get_allocator());
		...
		if((allocationSide == kSideBack) && (nAdditionalCapacity <= nUnusedPtrCountAtFront))  // If we can take advantage of unused pointers at the front without doing any reallocation...
		{
			...
			tr.memmove(pPtrArrayBegin, mItBegin.mpCurrentArrayPtr, nUsedPtrSpace);
		...
		else if((allocationSide == kSideFront) && (nAdditionalCapacity <= nUnusedPtrCountAtBack)) // If we can take advantage of unused pointers at the back without doing any reallocation...
		{
			...
			#if EASTL_DEBUG
				memset(mpPtrArray, 0, size_t(uintptr_t(pPtrArrayBegin) - uintptr_t(static_cast<T **>(mpPtrArray))));
				// memset(mpPtrArray, 0, size_t(uintptr_t(pPtrArrayBegin) - uintptr_t(static_cast<MoveAssignable**>(mpPtrArray))));
			#endif
		}
		else
		{
			// In this case we will have to do a reallocation.
			const size_type    nNewPtrArraySize = mnPtrArraySize + eastl::max_alt(mnPtrArraySize.value(), nAdditionalCapacity) + 2;  // Allocate extra capacity.
			...
			{
				tr.memcpy(pPtrArrayBegin, mItBegin.mpCurrentArrayPtr, nUsedPtrSpace);
			}
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker, unsigned kDequeSubarraySize, bool kTracked>
	DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::DequeIterator()
		: mpCurrent(nullptr)
		, mpBegin(nullptr)
		, mpEnd(nullptr)
		, mpCurrentArrayPtr(nullptr)
	{
		// Empty
	}
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker, unsigned kDequeSubarraySize, bool kTracked>
	DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::DequeIterator(T** pCurrentArrayPtr, T* pCurrent)
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker, unsigned kDequeSubarraySize, bool kTracked>
	DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::DequeIterator(const iterator& x)
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker, unsigned kDequeSubarraySize, bool kTracked>
	DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::DequeIterator(const tracked_iterator& x)
		: mpCurrent(x.mpCurrent), mpBegin(x.mpBegin), mpEnd(x.mpEnd), mpCurrentArrayPtr(x.mpCurrentArrayPtr)
	{
		// Empty
	}
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker, unsigned kDequeSubarraySize, bool kTracked>
	DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::DequeIterator(const iterator& x, Increment)
		: mpCurrent(x.mpCurrent), mpBegin(x.mpBegin), mpEnd(x.mpEnd), mpCurrentArrayPtr(x.mpCurrentArrayPtr)
	{
		operator++();
	}
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker, unsigned kDequeSubarraySize, bool kTracked>
	DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::DequeIterator(const iterator& x, Decrement)
		: mpCurrent(x.mpCurrent), mpBegin(x.mpBegin), mpEnd(x.mpEnd), mpCurrentArrayPtr(x.mpCurrentArrayPtr)
	{
		operator--();
	}
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker, unsigned kDequeSubarraySize, bool kTracked>
	typename DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::pointer
	DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::operator->() const
	{
		return mpCurrent;
	}
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker, unsigned kDequeSubarraySize, bool kTracked>
	typename DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::reference
	DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::operator*() const
	{
		return *mpCurrent;
	}
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker, unsigned kDequeSubarraySize, bool kTracked>
	typename DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::this_type&
	DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::operator++()
	{
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker, unsigned kDequeSubarraySize, bool kTracked>
	typename DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::this_type
	DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::operator++(int)
	{
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker, unsigned kDequeSubarraySize, bool kTracked>
	typename DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::this_type&
	DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::operator+=(difference_type n)
	{
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker, unsigned kDequeSubarraySize, bool kTracked>
	typename DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::this_type
	DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::operator+(difference_type n) const
	{
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker, unsigned kDequeSubarraySize, bool kTracked>
	typename DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::this_type
	DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::copy(
			Tracker &tr, const iterator& first, const iterator& last, true_type)
	{
		...
		{
			mem_wrap(tr).memmove(mpCurrent, first.mpCurrent, (last.mpCurrent - first.mpCurrent) * (sizeof *last.mpCurrent));
	...
	typename DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::this_type
	DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::copy(
			Tracker &, const iterator& first, const iterator& last, false_type)
	{
		return eastl::copy(eastl::make_move_iterator(first), eastl::make_move_iterator(last), eastl::make_move_iterator(*this)).base();
	}
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker, unsigned kDequeSubarraySize, bool kTracked>
	void DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::copy_backward(
			Tracker &tr,
			const iterator& first, const iterator& last, true_type)
	{
		...
		{
			mem_wrap(tr).memmove(mpCurrent - (last.mpCurrent - first.mpCurrent), first.mpCurrent, (last.mpCurrent - first.mpCurrent) * (sizeof *last.mpCurrent));
		}
		else
	...
	void DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::copy_backward(
			Tracker &,
			const iterator& first, const iterator& last, false_type)
	{
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker, unsigned kDequeSubarraySize, bool kTracked>
	void DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>::SetSubarray(T** pCurrentArrayPtr)
	{
	...
	template <typename T, typename PointerA, typename ReferenceA, typename TrackerA, typename PointerB, typename ReferenceB, typename TrackerB, unsigned kDequeSubarraySize, bool kTracked>
	inline bool operator==(const DequeIterator<T, PointerA, ReferenceA, TrackerA, kDequeSubarraySize, kTracked>& a,
						   const DequeIterator<T, PointerB, ReferenceB, TrackerB, kDequeSubarraySize, kTracked>& b)
	{
		return a.mpCurrent == b.mpCurrent;
	}


	template <typename T, typename PointerA, typename ReferenceA, typename TrackerA, typename PointerB, typename ReferenceB, typename TrackerB, unsigned kDequeSubarraySize, bool kTrackedA, bool kTrackedB>
	inline bool operator!=(const DequeIterator<T, PointerA, ReferenceA, TrackerA, kDequeSubarraySize, kTrackedA>& a,
						   const DequeIterator<T, PointerB, ReferenceB, TrackerB, kDequeSubarraySize, kTrackedB>& b)
	{
		return a.mpCurrent != b.mpCurrent;
	}
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker, unsigned kDequeSubarraySize, bool kTracked>
	inline bool operator!=(const DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>& a,
						   const DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>& b)
	{
		return a.mpCurrent != b.mpCurrent;
	}
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker, unsigned kDequeSubarraySize, bool kTracked>
	inline DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>
	operator+(ptrdiff_t n, const DequeIterator<T, Pointer, Reference, Tracker, kDequeSubarraySize, kTracked>& x)
	{
	...
	template <typename T, typename PointerA, typename ReferenceA, typename TrackerA, typename PointerB, typename ReferenceB, typename TrackerB, unsigned kDequeSubarraySize, bool kTrackedA, bool kTrackedB>
	inline typename DequeIterator<T, PointerA, ReferenceA, TrackerA, kDequeSubarraySize, kTrackedA>::difference_type
	operator-(const DequeIterator<T, PointerA, ReferenceA, TrackerA, kDequeSubarraySize, kTrackedA>& a,
			  const DequeIterator<T, PointerB, ReferenceB, TrackerB, kDequeSubarraySize, kTrackedB>& b)
	{
		// This is a fairly clever algorithm that has been used in STL deque implementations since the original HP STL:
		typedef typename DequeIterator<T, PointerA, ReferenceA, TrackerA, kDequeSubarraySize, kTrackedA>::difference_type difference_type;
	...
	inline deque<T, Allocator, kDequeSubarraySize>::~deque()
	{
		...
		for(internal_iterator itCurrent(mItBegin); &*itCurrent != &*mItEnd; ++itCurrent)
	...
	deque<T, Allocator, kDequeSubarraySize>::emplace(const_iterator position, Args&&... args)
	{
		...
		if(i < difference_type(size() / 2)) // Should we insert at the front or at the back? We divide the range in half.
		{
			...
			oldBegin.template copy(tr, oldBeginPlus1, newPosition, eastl::has_trivial_relocate<value_type>());
		}
		else
		{
			...
			oldBack.template copy_backward(
				tr, itPosition, oldBackMinus1, eastl::has_trivial_relocate<value_type>());
		}
	...
	deque<T, Allocator, kDequeSubarraySize>::erase(const_iterator position)
	{
		...
		{
			itNext.template copy_backward(
				tr, mItBegin, itPosition, eastl::has_trivial_relocate<value_type>());
			pop_front();
		}
		else
		{
			itPosition.template copy(tr, itNext, mItEnd, eastl::has_trivial_relocate<value_type>());
			pop_back();
		}
	...
	deque<T, Allocator, kDequeSubarraySize>::erase(const_iterator first, const_iterator last)
	{
		...
		if((itFirst != mItBegin) || (itLast != mItEnd)) // If not erasing everything... (We expect that the user won't call erase(begin, end) because instead the user would just call clear.)
		{
			...
			if(i < difference_type((size() - n) / 2)) // Should we move the front entries forward or the back entries backward? We divide the range in half.
			{
				...
				itLast.template copy_backward(
						tr, mItBegin, itFirst, eastl::has_trivial_relocate<value_type>());
			...
			else // Else we will be moving back entries backward.
			{
				...
				itFirst.template copy(tr, itLast, mItEnd, eastl::has_trivial_relocate<value_type>());
			...
			try
			{
				...
				for(pPtrArrayCurrent = mItBegin.mpCurrentArrayPtr; pPtrArrayCurrent < mItEnd.mpCurrentArrayPtr; ++pPtrArrayCurrent) // Copy to the known-to-be-completely-used subarrays.
				{
					...
					eastl::uninitialized_copy(
						...
						, non_const_iterator_type(current)
						, const_cast<non_const_value_type*>(*pPtrArrayCurrent)
					);
					first = current;
				}
	...
	void deque<T, Allocator, kDequeSubarraySize>::DoInsertFromIterator(const_iterator position, const InputIterator& first, const InputIterator& last, EASTL_ITC_NS::forward_iterator_tag)
	{
		...
		else
		{
			...
			{
				const internal_iterator itNewBegin(DoReallocSubarray(n, kSideFront)); // itNewBegin to mItBegin refers to memory that isn't initialized yet; so it's not truly a valid iterator. Or at least not a dereferencable one.
				const internal_iterator itOldBegin(mItBegin);
				const internal_iterator itPosition(mItBegin + nInsertionIndex); // We need to reset this value because the reallocation above can invalidate iterators.
					...
					try
					{
						...
						{
							internal_iterator itUCopyEnd(mItBegin + difference_type(n));
			...
			{
				const internal_iterator        itNewEnd(DoReallocSubarray(n, kSideBack));
				const internal_iterator        itOldEnd(mItEnd);
				const difference_type nPushedCount = difference_type(nSize) - nInsertionIndex;
				const internal_iterator        itPosition(mItEnd - nPushedCount); // We need to reset this value because the reallocation above can invalidate iterators.
					...
					try
					{
						...
						{
							const internal_iterator itUCopyEnd(mItEnd - difference_type(n));
	...
	void deque<T, Allocator, kDequeSubarraySize>::DoInsertValues(const_iterator position, size_type n, const value_type& value)
	{
		...
		{
			const internal_iterator itNewBegin(DoReallocSubarray(n, kSideFront));
			...
			else // Else the insertion index is in the back half of the deque, so grow the deque at the back.
			{
				...
				const internal_iterator        itPosition(mItEnd - nPushedCount); // We need to reset this value because the reallocation above can invalidate iterators.
					...
					try
					{
						...
						{
							internal_iterator itUCopyEnd(mItEnd - difference_type(n));
