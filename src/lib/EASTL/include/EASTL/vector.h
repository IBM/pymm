...
namespace eastl
{
	...
	protected:
		...
		auto const& internalCapacityPtr() const EA_NOEXCEPT { return mCapacityAllocator.first(); }
	...
	inline VectorBase<T, Allocator>::VectorBase()
		: mpBegin(NULL, Destructible<tracker_type>()),
		  mpEnd(NULL, Destructible<tracker_type>()),
		  mCapacityAllocator(weak_tracked<T*, tracker_type, 'V'>{Destructible<tracker_type>(), nullptr}, allocator_type(EASTL_VECTOR_DEFAULT_NAME))
	{
	}
	...
	inline VectorBase<T, Allocator>::VectorBase(const allocator_type& allocator)
		: mpBegin(nullptr, allocator),
		  mpEnd(nullptr, allocator),
		  mCapacityAllocator(weak_tracked<T*, tracker_type, 'V'>(allocator,nullptr), allocator)
	{
	}
	...
	inline VectorBase<T, Allocator>::VectorBase(size_type n, const allocator_type& allocator)
		: mpBegin(NULL, allocator),
		  mpEnd(NULL, allocator),
		  mCapacityAllocator(weak_tracked<T*, tracker_type, 'V'>(allocator, nullptr), allocator)
	{
		mpBegin    = DoAllocate(n);
		mpEnd      = mpBegin;
		internalCapacityPtr().assign(internalAllocator(), mpBegin + n);
	}
	...
	{
		if(mpBegin)
			EASTLFree(internalAllocator(), +mpBegin, size_type(internalCapacityPtr() - mpBegin) * sizeof(T));
	}
	...
	inline void vector<T, Allocator>::resize(size_type n)
	{
		...
		else
		{
			 ...
			 * Is this too many?
			 */
			eastl::destruct(mpBegin + n, mpEnd + 0);
			mpEnd = mpBegin + n;
		}
	}
	...
	template <typename T, typename Allocator>
		template <typename V>
		inline void vector<T, Allocator>::push_back(const V& value)
		{
			this->push_back(value_type(value, this->get_allocator()));
		}
	...
	template <typename T, typename Allocator>
		template <typename V>
		inline typename vector<T, Allocator>::iterator
		vector<T, Allocator>::insert(const_iterator position, const V & value)
		{
			return this->insert(position, value_type(value, this->get_allocator()));
		}
		...
		if (first != last)
		{
			iterator const position =
				const_cast<value_type*>(
					eastl::move(const_cast<value_type*>(last), const_cast<value_type*>(+mpEnd), const_cast<value_type*>(first))
				);
	...
	inline void vector<T, Allocator>::reset_lose_memory() EA_NOEXCEPT
	{
		...
		mpBegin = mpEnd = internalCapacityPtr().assign(internalAllocator(), nullptr);
	}
	...
	inline void vector<T, Allocator>::DoInitFromIterator(ForwardIterator first, ForwardIterator last, EASTL_ITC_NS::forward_iterator_tag)
	{
		...
		typedef typename eastl::remove_const<T>::type non_const_value_type; // If T is a const type (e.g. const int) then we need to initialize it as if it were non-const.
		eastl::uninitialized_copy_ptr(first, last, const_cast<non_const_value_type*>(static_cast<value_type*>(+mpBegin)));
	}
	...
	void vector<T, Allocator>::DoAssignFromIterator(RandomAccessIterator first, RandomAccessIterator last, EASTL_ITC_NS::random_access_iterator_tag)
	{
		...
		if(n > size_type(internalCapacityPtr() - mpBegin)) // If n > capacity ...
		{
			...
			mpBegin    = pNewData;
			mpEnd      = mpBegin + n;
			internalCapacityPtr().assign(internalAllocator(), mpEnd);
		}
		else if(n <= size_type(mpEnd - mpBegin)) // If n <= size ...
		{
	...
	void vector<T, Allocator>::DoInsertFromIterator(const_iterator position, BidirectionalIterator first, BidirectionalIterator last, EASTL_ITC_NS::bidirectional_iterator_tag)
	{
		...
		if(first != last)
		{
			...
			else // else we need to expand our capacity.
			{
				...
				mpBegin    = pNewData;
				mpEnd      = pNewEnd;
				internalCapacityPtr().assign(internalAllocator(), pNewData + nNewSize);
			}
		}
	}
	...
	void vector<T, Allocator>::DoGrow(size_type n)
	{
		...
		mpBegin    = pNewData;
		mpEnd      = pNewEnd;
		internalCapacityPtr().assign(internalAllocator(), pNewData + n);
	}
