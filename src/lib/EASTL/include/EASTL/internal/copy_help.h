...
#if 1
/* memmove suberts typing, so using it with "tracked" data would require
 * a call to the tracker. But it might also be used with untracked data.
 * Simplest to disable the optimization for now.
 */
	...
	struct move_and_copy_helper<EASTL_ITC_NS::random_access_iterator_tag, isMove, true>
	{
		...
		static T* move_or_copy(const T* first, const T* last, T* result)
		{
			...
			memmove(result, first, size_t(uintptr_t(last) - uintptr_t(first)));
