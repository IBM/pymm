...
namespace eastl
{
	...
	/// 
	template <typename Value, typename Tracker, bool bCacheHashCode>
	struct hash_node;
	...
	EA_DISABLE_VC_WARNING(4625 4626) // "copy constructor / assignment operator could not be generated because a base class copy constructor is inaccessible or deleted"
		...
		struct hash_node<Value, Tracker, true>
		{
			hash_node() = default;
		...
		template <typename Value, typename Tracker>
		struct hash_node<Value, Tracker, false>
		{
			hash_node() = default;
			hash_node(const hash_node&) = default;
			hash_node(hash_node&&) = default;
	...
	static_assert(Internal::has_hashcode_member<hash_node<int, DummyTracker, true>>::value, "contains a mnHashCode member");
	static_assert(!Internal::has_hashcode_member<hash_node<int, DummyTracker, false>>::value, "doesn't contain a mnHashCode member");
	...
	{
		typedef hash_node<Value, Tracker, bCacheHashCode> node_type;
	...
	/// or an iterator.
	///
	template <typename Value, typename Tracker, bool bConst, bool bCacheHashCode>
	struct node_iterator : public node_iterator_base<Value, Tracker, bCacheHashCode>
	{
	public:
		typedef node_iterator_base<Value, Tracker, bCacheHashCode>     base_type;
		typedef node_iterator<Value, Tracker, bConst, bCacheHashCode>  this_type;
	...
	public:
		...
		node_iterator(const node_iterator<Value, Tracker, true, bCacheHashCode>& x)
			: base_type(x.mpNode) { }
	...
	{
	public:
		typedef hashtable_iterator_base<Value, Tracker, bCacheHashCode> this_type;
		typedef hash_node<Value, Tracker, bCacheHashCode>    node_type;
	...
	protected:
		...
		template <typename V, typename A, bool b>
		friend bool operator==(const hashtable_iterator_base<V, A, b>&, const hashtable_iterator_base<V, A, b>&);
	...
	///
	template <typename Value, typename Tracker, bool bConst, bool bCacheHashCode>
	struct hashtable_iterator : public hashtable_iterator_base<Value, Tracker, bCacheHashCode>
	{
	public:
		typedef hashtable_iterator_base<Value, Tracker, bCacheHashCode> base_type;
		typedef hashtable_iterator<Value, Tracker, bConst, bCacheHashCode> this_type;
		typedef hashtable_iterator<Value, Tracker, false, bCacheHashCode> this_type_non_const;
		typedef typename base_type::node_type                            node_type;
		typedef Value                                                    value_type;
		typedef typename type_select<bConst, const Value*, Value*>::type pointer;
		typedef typename type_select<bConst, const Value&, Value&>::type reference;
	...
	///
	template <typename Key, typename Value, typename Tracker, typename ExtractKey, typename Equal,
	...
	///
	template <typename Key, typename Value, typename Tracker, typename ExtractKey, typename Equal, typename H1, typename H2, typename H>
	struct hash_code_base<Key, Value, Tracker, ExtractKey, Equal, H1, H2, H, false>
	{
	protected:
	...
	protected:
		...
		bucket_index_t bucket_index(const hash_node<Value, Tracker, false>* pNode, uint32_t nBucketCount) const
			{ return (bucket_index_t)mRangedHash(mExtractKey(pNode->mValue), nBucketCount); }

		bool compare(const Key& key, hash_code_t, hash_node<Value, Tracker, false>* pNode) const
			{ return mEqual(key, mExtractKey(pNode->mValue)); }

		void copy_code(hash_node<Value, Tracker, false>*, const hash_node<Value, Tracker, false>*) const
			{ } // Nothing to do.

		void set_code(hash_node<Value, Tracker, false>* pDest, hash_code_t c) const
		{
			EA_UNUSED(pDest);
			EA_UNUSED(c);
		}
	...
	///
	template <typename Key, typename Value, typename Tracker, typename ExtractKey, typename Equal, typename H1, typename H2, typename H>
	struct hash_code_base<Key, Value, Tracker, ExtractKey, Equal, H1, H2, H, true>;
	...
	///
	template <typename Key, typename Value, typename Tracker, typename ExtractKey, typename Equal, typename H1, typename H2>
	struct hash_code_base<Key, Value, Tracker, ExtractKey, Equal, H1, H2, default_ranged_hash, false>
	{
	protected:
	...
	protected:
		...
		typedef uint32_t bucket_index_t;
		typedef hash_node<Value, Tracker, false> node_type;
	...
	///
	template <typename Key, typename Value, typename Tracker, typename ExtractKey, typename Equal, typename H1, typename H2>
	struct hash_code_base<Key, Value, Tracker, ExtractKey, Equal, H1, H2, default_ranged_hash, true>
	{
	protected:
		ExtractKey  mExtractKey;
		Equal       mEqual;
	...
	protected:
		...
		typedef uint32_t bucket_index_t;
		typedef hash_node<Value, Tracker, true> node_type;
	...
	class hashtable
		:   public rehash_base<RehashPolicy, hashtable<Key, Value, Allocator, ExtractKey, Equal, H1, H2, H, RehashPolicy, bCacheHashCode, bMutableIterators, bUniqueKeys> >,
			public hash_code_base<Key, Value, typename Allocator::tracker_type, ExtractKey, Equal, H1, H2, H, bCacheHashCode>
	{
	public:
		...
		typedef hash_code_base<Key, Value, tracker_type, ExtractKey, Equal, H1, H2, H, bCacheHashCode> hash_code_base_type;
		...
		typedef node_iterator<value_type, tracker_type, !bMutableIterators, bCacheHashCode>         local_iterator;
		typedef node_iterator<value_type, tracker_type, true,               bCacheHashCode>         const_local_iterator;
		typedef hashtable_iterator<value_type, tracker_type, !bMutableIterators, bCacheHashCode>    iterator;
		typedef hashtable_iterator<value_type, tracker_type, true,               bCacheHashCode>    const_iterator;
		typedef hash_node<value_type, tracker_type, bCacheHashCode>                                 node_type;
	...
	public:
		...
		bool empty() const EA_NOEXCEPT
			{ return mnElementCount == size_type(0); }
	...
	template <typename Value, typename Tracker, bool bCacheHashCode>
	inline bool operator==(const node_iterator_base<Value, Tracker, bCacheHashCode>& a, const node_iterator_base<Value, Tracker, bCacheHashCode>& b)
	...
	template <typename Value, typename Tracker, bool bCacheHashCode>
	inline bool operator==(const hashtable_iterator_base<Value, Tracker, bCacheHashCode>& a, const hashtable_iterator_base<Value, Tracker, bCacheHashCode>& b)
		{ return a.mpNode == b.mpNode; }
	...
	::hashtable(size_type nBucketCount, const H1& h1, const H2& h2, const H& h,
		...
		:   rehash_base<RP, hashtable>(),
			hash_code_base<K, V, tracker_type, EK, Eq, H1, H2, H, bC>(ek, eq, h1, h2, h),
			mnBucketCount(0),
			mnElementCount(0),
			mRehashPolicy(),
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::hashtable(FowardIterator first, FowardIterator last, size_type nBucketCount, 
		...
		:   rehash_base<rehash_policy_type, hashtable>(),
			hash_code_base<key_type, value_type, tracker_type, extract_key_type, key_equal, h1_type, h2_type, h_type, kCacheHashCode>(ek, eq, h1, h2, h),
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::hashtable(const this_type& x)
		:   rehash_base<RP, hashtable>(x),
			hash_code_base<K, V, tracker_type, EK, Eq, H1, H2, H, bC>(x),
					...
					for(size_type i = 0; i < x.mnBucketCount; ++i)
					{
						...
						while(pNodeSource)
						{
							*ppNodeDest = DoAllocateNode(pNodeSource->mValue);
							auto m = ((*ppNodeDest)->mpNext).make_modifier();
							copy_code(*ppNodeDest, pNodeSource);
							ppNodeDest = &(*ppNodeDest)->mpNext.value(m);
	...
	{
		hash_code_base<K, V, tracker_type, EK, Eq, H1, H2, H, bC>::base_swap(x); // hash_code_base has multiple implementations, so we let them handle the swap.
		...
		if(pNode == NULL) // If value is not present... add it.
		{
			...
			set_code(pNodeNew, c); // This is a no-op for most hashtables.
				...
				try
				{
					...
					EASTL_ASSERT((uintptr_t)static_cast<node_type**>(mpBucketArray) != (uintptr_t)&gpEmptyBucketArray[0]);
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValue(BoolConstantT, Args&&... args) // false_type means bUniqueKeys is false.
	{
		...
		if(pNodePrev == NULL)
		{
			EASTL_ASSERT((void**)static_cast<node_type**>(mpBucketArray) != &gpEmptyBucketArray[0]);
			pNodeNew->mpNext = mpBucketArray[n];
			mpBucketArray[n] = pNodeNew;
		}
		else
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::erase(const key_type& k)
	{
		...
		{
			auto m = ((*pBucketArray)->mpNext).make_modifier();
			pBucketArray = &((*pBucketArray)->mpNext).value(m);
		}
	...
	inline void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::reset_lose_memory() EA_NOEXCEPT
	{
		...
		mnBucketCount  = 1;
			...
			tr.memcpy(&mpBucketArray, &p, sizeof(mpBucketArray)); // Other compilers implement strict aliasing and casting is thus unsafe.
		#endif
