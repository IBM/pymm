...
namespace eastl
{
	...
	inline T* CharStringUninitializedCopy(Tracker &a, const T* pSource, const T* pSourceEnd, T* pDestination)
	{
		mem_wrap(a).memmove(pDestination, pSource, (size_t)(pSourceEnd - pSource) * sizeof(T));
	...
	inline char8_t* CharStringUninitializedFillN(Allocator &a, char8_t* const pDestination, size_t n, const char8_t c)
	{
		if(n) // Some compilers (e.g. GCC 4.3+) generate a warning (which can't be disabled) if you call memset with a size of 0.
			mem_wrap(a).memset(pDestination, (uint8_t)c, (size_t)n);
		return pDestination + n;
	}

	template<typename T, typename Allocator>
	inline T* CharStringUninitializedFillN(Allocator &a, T* const pDestination, size_t n, const T c)
	{
	...
	template<typename Allocator>
	inline char8_t* CharTypeAssignN(Allocator &a, char8_t* const pDestination, size_t n, char8_t c)
	{
		if(n) // Some compilers (e.g. GCC 4.3+) generate a warning (which can't be disabled) if you call memset with a size of 0.
			return (char8_t*)mem_wrap(a).memset(pDestination, c, (size_t)n);
		return pDestination;
	}

	template<typename T, typename Allocator>
	inline T* CharTypeAssignN(Allocator &a, T* const pDestination, size_t n, T c)
	{
		T* pDest            = pDestination;
		const T* const pEnd = pDestination + n;
