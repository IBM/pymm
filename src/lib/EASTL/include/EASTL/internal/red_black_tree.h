...
namespace eastl
{
	...
	struct rbtree_node : public rbtree_node_base<Tracker>
	{
		...
		#endif
		/* For tracking purposes (and contrarty the comment above, the rbtree_node now *is* constructed */
		rbtree_node(const Tracker &t)
			: rbtree_node_base<Tracker>(t)
			, mValue{}
		{}
	};
	...
	//
	template <typename Value, typename Tracker>
	rbtree_node_base<Tracker>* RBTreeIncrement    (const rbtree_node_base<Tracker>* pNode);
	template <typename Value, typename Tracker>
	rbtree_node_base<Tracker>* RBTreeDecrement    (const rbtree_node_base<Tracker>* pNode);
	template <typename Value, typename Tracker>
	rbtree_node_base<Tracker>* RBTreeGetMinChild  (const rbtree_node_base<Tracker>* pNode);
	template <typename Value, typename Tracker>
	rbtree_node_base<Tracker>* RBTreeGetMaxChild  (const rbtree_node_base<Tracker>* pNode);
	template <typename Value, typename Tracker>
	size_t            RBTreeGetBlackCount(const rbtree_node_base<Tracker>* pNodeTop,
													const rbtree_node_base<Tracker>* pNodeBottom);
	template <typename Value, typename Tracker>
	void              RBTreeInsert       (      rbtree_node_base<Tracker>* pNode,
														  rbtree_node_base<Tracker>* pNodeParent,
														  rbtree_node_base<Tracker>* pNodeAnchor,
														  RBTreeSide insertionSide);
	template <typename Value, typename Tracker>
	void              RBTreeErase        (      rbtree_node_base<Tracker>* pNode,
														  rbtree_node_base<Tracker>* pNodeAnchor);
	...
	{
		typedef rbtree_iterator<T, Pointer, Reference, Tracker> this_type;
		typedef rbtree_iterator<T, T*, T&, Tracker>         iterator;
		typedef rbtree_iterator<T, const T*, const T&, Tracker> const_iterator;
		typedef eastl_size_t                                size_type;     // See config.h for the definition of eastl_size_t, which defaults to size_t.
		typedef ptrdiff_t                                   difference_type;
		typedef T                                           value_type;
		typedef rbtree_node_base<Tracker>                   base_node_type;
		typedef rbtree_node<T, Tracker>                     node_type;
	...
	public:
		...
		typedef rbtree_node<value_type, tracker_type>                                           node_type;
		...
		typedef typename type_select<bMutableIterators,
					rbtree_iterator<value_type, value_type*, value_type&, tracker_type>,
					rbtree_iterator<value_type, const value_type*, const value_type&, tracker_type> >::type iterator;
		typedef rbtree_iterator<value_type, const value_type*, const value_type&, tracker_type>    const_iterator;
	...
	public:
		rbtree_node_base<tracker_type> mAnchor;      /// This node acts as end() and its mpLeft points to begin(), and mpRight points to rbegin() (the last node on the right).
	...
	template <typename Tracker>
	inline rbtree_node_base<Tracker>* RBTreeGetMinChild(const rbtree_node_base<Tracker>* pNodeBase)
	{
		while(pNodeBase->mpNodeLeft) 
			pNodeBase = pNodeBase->mpNodeLeft;
		return const_cast<rbtree_node_base<Tracker>*>(pNodeBase);
	}

	template <typename Tracker>
	inline rbtree_node_base<Tracker>* RBTreeGetMaxChild(const rbtree_node_base<Tracker>* pNodeBase)
	{
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	rbtree_iterator<T, Pointer, Reference, Tracker>::rbtree_iterator()
		: mpNode(NULL) { }


	template <typename T, typename Pointer, typename Reference, typename Tracker>
	rbtree_iterator<T, Pointer, Reference, Tracker>::rbtree_iterator(const node_type* pNode)
		: mpNode(static_cast<node_type*>(const_cast<node_type*>(pNode))) { }


	template <typename T, typename Pointer, typename Reference, typename Tracker>
	rbtree_iterator<T, Pointer, Reference, Tracker>::rbtree_iterator(const iterator& x)
		: mpNode(x.mpNode) { }

	template <typename T, typename Pointer, typename Reference, typename Tracker>
	rbtree_iterator<T, Pointer, Reference, Tracker> &
	rbtree_iterator<T, Pointer, Reference, Tracker>::operator=(const iterator& x)
	{
		mpNode = x.mpNode;
		return *this;
	}
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	typename rbtree_iterator<T, Pointer, Reference, Tracker>::reference
	rbtree_iterator<T, Pointer, Reference, Tracker>::operator*() const
		{ return mpNode->mValue; }


	template <typename T, typename Pointer, typename Reference, typename Tracker>
	typename rbtree_iterator<T, Pointer, Reference, Tracker>::pointer
	rbtree_iterator<T, Pointer, Reference, Tracker>::operator->() const
		{ return &mpNode->mValue; }


	template <typename T, typename Pointer, typename Reference, typename Tracker>
	typename rbtree_iterator<T, Pointer, Reference, Tracker>::this_type&
	rbtree_iterator<T, Pointer, Reference, Tracker>::operator++()
	{
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	typename rbtree_iterator<T, Pointer, Reference, Tracker>::this_type
	rbtree_iterator<T, Pointer, Reference, Tracker>::operator++(int)
	{
		this_type temp(*this);
		mpNode = static_cast<node_type*>(RBTreeIncrement(mpNode));
		return temp;
	...
	inline bool operator==(const rbtree_iterator<T, PointerA, ReferenceA, TrackerA>& a,
						   const rbtree_iterator<T, PointerB, ReferenceB, TrackerB>& b)
	{
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	inline bool operator!=(const rbtree_iterator<T, Pointer, Reference, Tracker>& a,
						   const rbtree_iterator<T, Pointer, Reference, Tracker>& b)
	{
		return a.mpNode != b.mpNode;
	}
		  ...
		  mAllocator(x.mAllocator)
	{
		...
		{
			mAnchor.mpNodeParent = DoCopySubtree(static_cast<const node_type*>(&*x.mAnchor.mpNodeParent), static_cast<node_type*>(&mAnchor));
			mAnchor.mpNodeRight  = RBTreeGetMaxChild(mAnchor.mpNodeParent.value());
			mAnchor.mpNodeLeft   = RBTreeGetMinChild(mAnchor.mpNodeParent.value());
			mnSize               = x.mnSize;
		}
	}
	...
	rbtree<K, V, C, A, E, bM, bU>::begin() const EA_NOEXCEPT
		{ return const_iterator(static_cast<node_type*>(const_cast<rbtree_node_base<tracker_type>*>(&*mAnchor.mpNodeLeft))); }
	...
	rbtree<K, V, C, A, E, bM, bU>::end() const EA_NOEXCEPT
		{ return const_iterator(static_cast<node_type*>(const_cast<rbtree_node_base<tracker_type>*>(&mAnchor))); }
	...
	rbtree<K, V, C, A, E, bM, bU>::DoGetKeyInsertionPositionUniqueKeys(bool& canInsert, const key_type& key)
	{
		...
		node_type* pCurrent    = static_cast<node_type*>(&*mAnchor.mpNodeParent); // Start with the root node.
		node_type* pLowerBound = static_cast<node_type*>(&mAnchor);             // Set it to the container end for now.
		...
		{
			if(EASTL_LIKELY(pLowerBound != static_cast<node_type*>(&*mAnchor.mpNodeLeft))) // If the tree was empty or if we otherwise need to insert at the very front of the tree...
			{
				...
				pLowerBound = static_cast<node_type*>(RBTreeDecrement(pLowerBound));
			}
			else
			{
				canInsert = true;
	...
	rbtree<K, V, C, A, E, bM, bU>::DoGetKeyInsertionPositionNonuniqueKeys(const key_type& key)
	{
		...
		node_type* pCurrent  = static_cast<node_type*>(&*mAnchor.mpNodeParent); // Start with the root node.
		node_type* pRangeEnd = static_cast<node_type*>(&mAnchor);             // Set it to the container end for now.
		extract_key extractKey;
	...
	rbtree<K, V, C, A, E, bM, bU>::DoGetKeyInsertionPositionUniqueKeysHint(const_iterator position, bool& bForceToLeft, const key_type& key)
	{
		...
		if(mnSize && mCompare(extractKey((static_cast<node_type*>(&*mAnchor.mpNodeRight))->mValue), key))
		{
			EASTL_VALIDATE_COMPARE(!mCompare(key, extractKey((static_cast<node_type*>(&*mAnchor.mpNodeRight))->mValue))); // Validate that the compare function is sane.
			bForceToLeft = false;
			return static_cast<node_type*>(&*mAnchor.mpNodeRight);
		}
	...
	rbtree<K, V, C, A, E, bM, bU>::DoGetKeyInsertionPositionNonuniqueKeysHint(const_iterator position, bool& bForceToLeft, const key_type& key)
	{
		...
		if(mnSize && !mCompare(key, extractKey((static_cast<node_type*>(&*mAnchor.mpNodeRight))->mValue))) // If we are non-empty and the value is >= the last node...
		{
			bForceToLeft =false;
			return static_cast<node_type*>(&*mAnchor.mpNodeRight);
		}
	...
	inline void rbtree<K, V, C, A, E, bM, bU>::reset_lose_memory()
	{
		...
		mAnchor.mpNodeLeft   = &mAnchor;
		mAnchor.mpNodeParent = NULL;
		mAnchor.mColor       = RBTreeColor::kRed;
		mnSize               = 0;
	}
	...
	rbtree<K, V, C, A, E, bM, bU>::erase(const_iterator first, const_iterator last)
	{
		...
		clear();
		return iterator(static_cast<node_type*>(&mAnchor)); // Same as: return end();
	}
	...
	bool rbtree<K, V, C, A, E, bM, bU>::validate() const
	{
		...
		if(mnSize)
		{
			...
			if(mAnchor.mpNodeLeft != RBTreeGetMinChild(mAnchor.mpNodeParent.value()))
				return false;

			if(mAnchor.mpNodeRight != RBTreeGetMaxChild(mAnchor.mpNodeParent.value()))
				return false;

			const size_t nBlackCount   = RBTreeGetBlackCount(mAnchor.mpNodeParent.value(), mAnchor.mpNodeLeft.value());
			size_type    nIteratedSize = 0;

			for(const_iterator it = begin(); it != end(); ++it, ++nIteratedSize)
			{
				const node_type* const pNode      = static_cast<const node_type*>(it.mpNode);
				const node_type* const pNodeRight = static_cast<const node_type*>(&*pNode->mpNodeRight);
				const node_type* const pNodeLeft  = static_cast<const node_type*>(&*pNode->mpNodeLeft);
				...
				// Verify item #1 above.
				if((pNode->mColor != RBTreeColor::kRed) && (pNode->mColor != RBTreeColor::kBlack))
					return false;

				// Verify item #3 above.
				if(pNode->mColor == RBTreeColor::kRed)
				{
					if((pNodeRight && (pNodeRight->mColor == RBTreeColor::kRed)) ||
					   (pNodeLeft  && (pNodeLeft->mColor  == RBTreeColor::kRed)))
						return false;
				}
				...
				{
					// Verify item #4 above.
					if(RBTreeGetBlackCount(mAnchor.mpNodeParent.value(), pNode) != nBlackCount)
						return false;
				}
			}
	...
	{
		auto* p = allocate_memory(mAllocator, sizeof(node_type), EASTL_ALIGN_OF(value_type), 0);
		EASTL_ASSERT_MSG(p != nullptr, "the behaviour of eastl::allocators that return nullptr is not defined.");
		auto *pNode = new (p) node_type(mAllocator);
		return pNode;
	}
	...
	rbtree<K, V, C, A, E, bM, bU>::DoCreateNodeFromKey(const key_type& key)
	{
		...
		node_type* const pNode = DoAllocateNode();
			...
			pNode->mColor       = RBTreeColor::kBlack;
		#endif
	...
	rbtree<K, V, C, A, E, bM, bU>::DoCopySubtree(const node_type* pNodeSource, node_type* pNodeDest)
	{
		node_type* const pNewNodeRoot = DoCreateNode(pNodeSource, pNodeDest);
			...
			try
			{
				...
				for(pNodeSource = static_cast<node_type*>(&*pNodeSource->mpNodeLeft), pNodeDest = pNewNodeRoot;
					pNodeSource;
					pNodeSource = static_cast<node_type*>(&*pNodeSource->mpNodeLeft), pNodeDest = pNewNodeLeft)
				{
	...
	void rbtree<K, V, C, A, E, bM, bU>::DoNukeSubtree(node_type* pNode)
	{
		...
		{
			DoNukeSubtree(static_cast<node_type *>(&*pNode->mpNodeRight));
...
#include <EASTL/internal/red_black_tree.tcc>
