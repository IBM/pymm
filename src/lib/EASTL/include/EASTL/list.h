...
namespace eastl
{
	...
	struct ListNodeBase
	{
		...
		void        splice(ListNodeBase<Tracker>* pFirst, ListNodeBase<Tracker>* pLast) EA_NOEXCEPT;          // Removes [pFirst,pLast) from the list it's in and inserts it before this in this node's list.
		void        reverse() EA_NOEXCEPT;                                                  // Reverses the order of nodes in the circular list this node is a part of.
		static void swap(ListNodeBase<Tracker>& a, ListNodeBase<Tracker>& b) EA_NOEXCEPT;                     // Swaps the nodes a and b in the lists to which they belong.
	...
	struct ListIterator
	{
		typedef ListIterator<T, Pointer, Reference, Tracker> this_type;
		typedef ListIterator<T, T*, T&, Tracker>             iterator;
		typedef ListIterator<T, const T*, const T&, Tracker> const_iterator;
		typedef eastl_size_t                                size_type;     // See config.h for the definition of eastl_size_t, which defaults to size_t.
		typedef ptrdiff_t                                   difference_type;
		typedef T                                           value_type;
		typedef ListNode<T, Tracker>                        node_type;
	...
	public:
		...
		ListIterator(const ListNodeBase<Tracker>* pNode) EA_NOEXCEPT;
		ListIterator(const iterator& x) EA_NOEXCEPT;
		/* iterator might be this_type, in which case a user-defined copy constructor
		 * implies the need for a user-defined operator=
		 */
		ListIterator &operator=(const iterator& x) EA_NOEXCEPT;
	...
	public:
		...
		typedef ListNode<T, tracker_type>            node_type;
		typedef eastl_size_t                         size_type;     // See config.h for the definition of eastl_size_t, which defaults to size_t.
		typedef ptrdiff_t                            difference_type;
		#if EASTL_LIST_PROXY_ENABLED
			typedef ListNodeBaseProxy< ListNode<T>, tracker_type > base_node_type;
		#else
			typedef ListNodeBase<tracker_type>                     base_node_type; // We use ListNodeBase instead of ListNode<T> because we don't want to create a T.
		#endif
	...
	public:
		...
		typedef ListIterator<T, T*, T&, tracker_type>   iterator;
		typedef ListIterator<T, const T*, const T&, tracker_type> const_iterator;
	...
	protected:
		...
		template <typename Integer>
		void DoInsert(ListNodeBase<tracker_type>* pNode, Integer n, Integer value, true_type);

		template <typename InputIterator>
		void DoInsert(ListNodeBase<tracker_type>* pNode, InputIterator first, InputIterator last, false_type);

		void DoInsertValues(ListNodeBase<tracker_type>* pNode, size_type n, const value_type& value);
	   
		template<typename... Args>
		void DoInsertValue(ListNodeBase<tracker_type>* pNode, Args&&... args);

		void DoErase(ListNodeBase<tracker_type>* pNode);
	...
	template <typename Tracker>
	inline void ListNodeBase<Tracker>::swap(ListNodeBase& a, ListNodeBase& b) EA_NOEXCEPT
	{
	...
	template <typename Tracker>
	inline void ListNodeBase<Tracker>::splice(ListNodeBase* first, ListNodeBase* last) EA_NOEXCEPT
	{
	...
	template <typename Tracker>
	inline void ListNodeBase<Tracker>::reverse() EA_NOEXCEPT
	{
	...
	template <typename Tracker>
	inline void ListNodeBase<Tracker>::insert(ListNodeBase* pNext) EA_NOEXCEPT
	{
		mpNext = pNext;
		mpPrev = pNext->mpPrev;
	...
	template <typename Tracker>
	inline void ListNodeBase<Tracker>::remove() EA_NOEXCEPT
	{
	...
	template <typename Tracker>
	inline void ListNodeBase<Tracker>::insert_range(ListNodeBase* pFirst, ListNodeBase* pFinal) EA_NOEXCEPT
	{
	...
	template <typename Tracker>
	inline void ListNodeBase<Tracker>::remove_range(ListNodeBase* pFirst, ListNodeBase* pFinal) EA_NOEXCEPT
	{
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	inline ListIterator<T, Pointer, Reference, Tracker>::ListIterator() EA_NOEXCEPT
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	inline ListIterator<T, Pointer, Reference, Tracker>::ListIterator(const ListNodeBase<Tracker>* pNode) EA_NOEXCEPT
		: mpNode(
			static_cast<node_type*>(
				static_cast<ListNode<T, Tracker>*>(const_cast<ListNodeBase<Tracker>*>(pNode))
			)
		) // All this casting is in the name of making runtime debugging much easier on the user.
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	inline ListIterator<T, Pointer, Reference, Tracker>::ListIterator(const iterator& x) EA_NOEXCEPT
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	ListIterator<T, Pointer, Reference, Tracker> &
	ListIterator<T, Pointer, Reference, Tracker>::operator=(const iterator& x) EA_NOEXCEPT
	{
		mpNode = const_cast<node_type*>(x.mpNode);
		return *this;
	}
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	inline typename ListIterator<T, Pointer, Reference, Tracker>::this_type
	ListIterator<T, Pointer, Reference, Tracker>::next() const EA_NOEXCEPT
	{
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	inline typename ListIterator<T, Pointer, Reference, Tracker>::this_type
	ListIterator<T, Pointer, Reference, Tracker>::prev() const EA_NOEXCEPT
	{
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	inline typename ListIterator<T, Pointer, Reference, Tracker>::reference
	ListIterator<T, Pointer, Reference, Tracker>::operator*() const EA_NOEXCEPT
	{
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	inline typename ListIterator<T, Pointer, Reference, Tracker>::pointer
	ListIterator<T, Pointer, Reference, Tracker>::operator->() const EA_NOEXCEPT
	{
		return &mpNode->mValue;
	}
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	inline typename ListIterator<T, Pointer, Reference, Tracker>::this_type&
	ListIterator<T, Pointer, Reference, Tracker>::operator++() EA_NOEXCEPT
	{
		mpNode = mpNode->mpNext.template ptr_cast<node_type>();
		return *this;
	}
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	inline typename ListIterator<T, Pointer, Reference, Tracker>::this_type
	ListIterator<T, Pointer, Reference, Tracker>::operator++(int) EA_NOEXCEPT
	{
		this_type temp(*this);
		mpNode = mpNode->mpNext.template ptr_cast<node_type>();
		return temp;
	...
	ListIterator<T, Pointer, Reference, Tracker>::operator--() EA_NOEXCEPT
	{
		mpNode = mpNode->mpPrev.template ptr_cast<node_type>();
		return *this;
	}
	...
	inline bool operator==(const ListIterator<T, PointerA, ReferenceA, TrackerA>& a,
						   const ListIterator<T, PointerB, ReferenceB, TrackerB>& b) EA_NOEXCEPT
	{
	...
	template <typename T, typename Pointer, typename Reference, typename Tracker>
	inline bool operator!=(const ListIterator<T, Pointer, Reference, Tracker>& a,
						   const ListIterator<T, Pointer, Reference, Tracker>& b) EA_NOEXCEPT
	{
		return a.mpNode != b.mpNode;
	}
	...
	{
		EASTL_ASSERT((mAllocator == allocator) || (mNode.mpNext.template ptr_cast<node_type>() == &mNode)); // We can only assign a different allocator if we are empty of elements.
		mAllocator = allocator;
	}
	...
	{
		mNode.mpNext = (ListNode<T, tracker_type>*)&mNode;
		mNode.mpPrev = (ListNode<T, tracker_type>*)&mNode;
	}
	...
	{
		node_type* p = mNode.mpNext.template ptr_cast<node_type>();

		while(p != &mNode)
		{
			node_type* const pTemp = p;
			p = p->mpNext.template ptr_cast<node_type>();
			pTemp->~node_type();
	...
	inline list<T, Allocator>::list(size_type n, const allocator_type& allocator)
		: base_type(allocator)
	{
		DoInsertValues((ListNodeBase<tracker_type>*)&mNode, n, value_type());
	}
	...
	inline list<T, Allocator>::list(size_type n, const value_type& value, const allocator_type& allocator)
		: base_type(allocator)
	{
		DoInsertValues((ListNodeBase<tracker_type>*)&mNode, n, value);
	}
	...
	{
		DoInsert((ListNodeBase<tracker_type>*)&mNode, const_iterator((ListNodeBase<tracker_type>*)x.mNode.mpNext), const_iterator((ListNodeBase<tracker_type>*)&x.mNode), false_type());
	}
	...
	inline list<T, Allocator>::list(std::initializer_list<value_type> ilist, const allocator_type& allocator)
		: base_type(allocator)
	{
		DoInsert((ListNodeBase<tracker_type>*)&mNode, ilist.begin(), ilist.end(), false_type());
	}
	...
	{
		//insert(const_iterator((ListNodeBase<tracker_type>*)&mNode), first, last);
		DoInsert((ListNodeBase<tracker_type>*)&mNode, first, last, is_integral<InputIterator>());
	}
	...
	{
		return iterator((ListNodeBase<tracker_type>*)mNode.mpNext);
	}
	...
	{
		return const_iterator((ListNodeBase<tracker_type>*)mNode.mpNext);
	}
	...
	{
		return iterator((ListNodeBase<tracker_type>*)&mNode);
	}
	...
	{
		return const_iterator((ListNodeBase<tracker_type>*)&mNode);
	}
	...
	{
		return reverse_iterator((ListNodeBase<tracker_type>*)&mNode);
	}
	...
	{
		return const_reverse_iterator((ListNodeBase<tracker_type>*)&mNode);
	}
	...
	{
		return reverse_iterator((ListNodeBase<tracker_type>*)mNode.mpNext);
	}
	...
	{
		return const_reverse_iterator((ListNodeBase<tracker_type>*)mNode.mpNext);
	}
	...
	list<T, Allocator>::front()
	{
			...
			if(EASTL_UNLIKELY(mNode.mpNext.template ptr_cast<node_type>() == &mNode))
				EASTL_FAIL_MSG("list::front -- empty container");
		#endif

		return mNode.mpNext.template ptr_cast<node_type>()->mValue;
	}
	...
	list<T, Allocator>::back()
	{
		...
		return mNode.mpPrev.template ptr_cast<node_type>()->mValue;
	}
	...
	inline bool list<T, Allocator>::empty() const EA_NOEXCEPT
	{
			...
			return (mSize == 0);
		#else
			return mNode.mpNext.template ptr_cast<node_type>() == &mNode;
		#endif
	}
	...
	list<T, Allocator>::size() const EA_NOEXCEPT
	{
		...
		#else
			#if EASTL_DEBUG
				const ListNodeBase<tracker_type>* p = (ListNodeBase<tracker_type>*)mNode.mpNext;
				size_type n = 0;
				while(p != (ListNodeBase<tracker_type>*)&mNode)
				{
					++n;
					p = (ListNodeBase<tracker_type>*)p->mpNext;
				}
				return n;
			#else
				// The following optimizes to slightly better code than the code above.
				return size_type(eastl::distance(const_iterator((ListNodeBase<tracker_type>*)mNode.mpNext), const_iterator((ListNodeBase<tracker_type>*)&mNode)));
			#endif
		#endif
	}
	...
	{
		iterator current((ListNodeBase<tracker_type>*)mNode.mpNext);
		size_type i = 0;
		...
		if(i == n)
			erase(current, (ListNodeBase<tracker_type>*)&mNode);
		else
			insert((ListNodeBase<tracker_type>*)&mNode, n - i, value);
	}
	...
	{
		DoInsertValue((ListNodeBase<tracker_type>*)mNode.mpNext, eastl::forward<Args>(args)...);
	}
	...
	{
		DoInsertValue((ListNodeBase<tracker_type>*)&mNode, eastl::forward<Args>(args)...);
	}
	...
	list<T, Allocator>::push_front()
	{
		...
		((ListNodeBase<tracker_type>*)pNode)->insert((ListNodeBase<tracker_type>*)mNode.mpNext);
		#if EASTL_LIST_SIZE_CACHE
			++mSize;
		#endif
		return mNode.mpNext.template ptr_cast<node_type>()->mValue; // Same as return front();
	}
	...
	inline void list<T, Allocator>::pop_front()
	{
		...
		DoErase((ListNodeBase<tracker_type>*)mNode.mpNext);
	}
	...
	{
		node_type* const pNode = DoCreateNode();
		((ListNodeBase<tracker_type>*)pNode)->insert((ListNodeBase<tracker_type>*)&mNode);
		#if EASTL_LIST_SIZE_CACHE
			++mSize;
		#endif
		return mNode.mpPrev.template ptr_cast<node_type>()->mValue;  // Same as return back();
	}
	...
	inline void list<T, Allocator>::pop_back()
	{
		...
		DoErase((ListNodeBase<tracker_type>*)mNode.mpPrev);
	}
	...
	list<T, Allocator>::insert(const_iterator position)
	{
		...
		((ListNodeBase<tracker_type>*)pNode)->insert((ListNodeBase<tracker_type>*)position.mpNode);
		#if EASTL_LIST_SIZE_CACHE
			++mSize;
		#endif
	...
	list<T, Allocator>::insert(const_iterator position, size_type n, const value_type& value)
	{
		...
		--itPrev;
		DoInsertValues((ListNodeBase<tracker_type>*)position.mpNode, n, value);
	...
	list<T, Allocator>::insert(const_iterator position, InputIterator first, InputIterator last)
	{
		iterator itPrev(position.mpNode);
		--itPrev;
		DoInsert((ListNodeBase<tracker_type>*)position.mpNode, first, last, is_integral<InputIterator>());
		return ++itPrev; // Inserts in front of position, returns iterator to new elements. 
	}
	...
	list<T, Allocator>::insert(const_iterator position, std::initializer_list<value_type> ilist)
	{
		iterator itPrev(position.mpNode);
		--itPrev;
		DoInsert((ListNodeBase<tracker_type>*)position.mpNode, ilist.begin(), ilist.end(), false_type());
		return ++itPrev; // Inserts in front of position, returns iterator to new elements. 
	}
	...
	{
		++position;
		DoErase((ListNodeBase<tracker_type>*)position.mpNode->mpPrev);
	...
	void list<T, Allocator>::remove(const value_type& value)
	{
		...
		while(current.mpNode != &mNode)
		{
			...
			{
				++current;
				DoErase((ListNodeBase<tracker_type>*)current.mpNode->mpPrev);
			}
		}
	}
	...
	{
		for(iterator first((ListNodeBase<tracker_type>*)mNode.mpNext), last((ListNodeBase<tracker_type>*)&mNode); first != last; )
		{
			iterator temp(first);
			++temp;
			if(predicate(first.mpNode->mValue))
				DoErase((ListNodeBase<tracker_type>*)first.mpNode);
			first = temp;
		}
	}
	...
	{
		((ListNodeBase<tracker_type>&)mNode).reverse();
	}
	...
	inline void list<T, Allocator>::splice(const_iterator position, this_type& x)
	{
		...
		if(mAllocator == x.mAllocator)
		{
				...
				if(x.mSize)
				{
					((ListNodeBase<tracker_type>*)position.mpNode)->splice((ListNodeBase<tracker_type>*)x.mNode.mpNext, (ListNodeBase<tracker_type>*)&x.mNode);
					mSize += x.mSize;
					x.mSize = 0;
				}
			#else
	...
	inline void list<T, Allocator>::splice(const_iterator position, list& x, const_iterator i)
	{
		if(mAllocator == x.mAllocator)
		{
			...
			{
				((ListNodeBase<tracker_type>*)position.mpNode)->splice((ListNodeBase<tracker_type>*)i.mpNode, (ListNodeBase<tracker_type>*)i2.mpNode);
	...
	inline void list<T, Allocator>::splice(const_iterator position, this_type& x, const_iterator first, const_iterator last)
	{
		if(mAllocator == x.mAllocator)
		{
				...
				if(n)
				{
					((ListNodeBase<tracker_type>*)position.mpNode)->splice((ListNodeBase<tracker_type>*)first.mpNode, (ListNodeBase<tracker_type>*)last.mpNode);
					mSize += n;
					x.mSize -= n;
				}
			#else
	...
	void list<T, Allocator>::unique()
	{
		...
		if(first != last)
		{
			...
			{
				if(*first == *next)
					DoErase((ListNodeBase<tracker_type>*)next.mpNode);
				else
					first = next;
				next = first;
			}
	...
	list<T, Allocator>::DoSort(iterator i1, iterator end2, size_type n, Compare& compare)
	{
		...
		if(compare(*i2, *i1))
		{
			...
			ListNodeBase<tracker_type>* i2Cut     = i2.mpNode;
			ListNodeBase<tracker_type>* i2CutLast = ix.mpNode->mpPrev;
			result = i2;
			end1   = i2 = ix;
			ListNodeBase<tracker_type>::remove_range(i2Cut, i2CutLast);
	...
	{
		node_type* pNode = mNode.mpNext.template ptr_cast<node_type>();

		for(; (pNode != &mNode) && (first != last); ++first)
		{
			pNode->mValue = *first;
			pNode         = pNode->mpNext.template ptr_cast<node_type>();
		}

		if(first == last)
			erase(const_iterator((ListNodeBase<tracker_type>*)pNode), (ListNodeBase<tracker_type>*)&mNode);
		else
			DoInsert((ListNodeBase<tracker_type>*)&mNode, first, last, false_type());
	}
	...
	template <typename T, typename Allocator>
	template <typename Integer>
	inline void list<T, Allocator>::DoInsert(ListNodeBase<tracker_type>* pNode, Integer n, Integer value, true_type)
	{
	...
	template <typename T, typename Allocator>
	template <typename InputIterator>
	inline void list<T, Allocator>::DoInsert(ListNodeBase<tracker_type>* pNode, InputIterator first, InputIterator last, false_type)
	{
	...
	template <typename T, typename Allocator>
	inline void list<T, Allocator>::DoInsertValues(ListNodeBase<tracker_type>* pNode, size_type n, const value_type& value)
	{
		for(; n > 0; --n)
	...
	template <typename T, typename Allocator>
	template<typename... Args>
	inline void list<T, Allocator>::DoInsertValue(ListNodeBase<tracker_type>* pNode, Args&&... args)
	{
		node_type* const pNodeNew = DoCreateNode(eastl::forward<Args>(args)...);
		((ListNodeBase<tracker_type>*)pNodeNew)->insert(pNode);
		#if EASTL_LIST_SIZE_CACHE
			++mSize;
		#endif
	}
	...
	template <typename T, typename Allocator>
	inline void list<T, Allocator>::DoErase(ListNodeBase<tracker_type>* pNode)
	{
		pNode->remove();
		...
		union
		{
			ListNodeBase<tracker_type>* mpBase;
			node_type*    mpNode;
		} node = { pNode };
	...
	{
		ListNodeBase<tracker_type>::swap((ListNodeBase<tracker_type>&)mNode, (ListNodeBase<tracker_type>&)x.mNode); // We need to implement a special swap because we can't do a shallow swap.
