...
/* This file defines literal operator suffix sv, which is system-reserved */
...
//
	...
	namespace eastl
	{
		...
		inline int Vsnprintf(char8_t* pDestination, size_t n, const char8_t* pFormat, va_list arguments)
			{
				/* A bad case for modification tracking. We do not know how many bytes
...
namespace eastl
{
	...
	protected:
		...
		// private except for use by fixed_string
		...
		public:
			...
			inline void SetHeapBeginPtr(const tracker_type &r, value_type* pBegin) EA_NOEXCEPT  { heap.mpBegin.assign(r, pBegin); }
			...
			{
				auto m = make_modifier(Destructible<tracker_type>(), &dst.raw, sizeof dst.raw);
				dst.raw = src.raw;
			}
			...
			inline void Swap(Allocator &c, Layout& a, Layout& b) EA_NOEXCEPT
			{
				auto ma = make_modifier(c, &a.raw, sizeof a.raw);
				auto mb = make_modifier(c, &b.raw, sizeof b.raw);
				eastl::swap(a.raw, b.raw);
			}

			inline void ResetToSSO() EA_NOEXCEPT
			{
				mem_wrap(Destructible<tracker_type>()).memset(&raw, 0, sizeof(RawLayout));
			}
		};
		...
		if((n < capacity() && internalLayout().IsHeap()) || (n > capacity()))
		{
			...
			if(EASTL_LIKELY(n))
			{
				...
				if(n <= SSOLayout::SSO_CAPACITY)
				{
					...
					CharStringUninitializedCopy(get_allocator(), pOldBegin, pOldBegin + n, internalLayout().BeginPtr());
				...
				internalLayout().SetHeapBeginPtr(internalAllocator(), pNewBegin);
	...
	inline void basic_string<T, Allocator>::clear() EA_NOEXCEPT
	{
		...
		auto m = make_modifier(get_allocator(), internalLayout().BeginPtr(), sizeof *internalLayout().BeginPtr());
	...
	basic_string<T, Allocator>::detach() EA_NOEXCEPT
	{
		...
		if (internalLayout().IsSSO())
		{
			...
			pointer pNewEnd = CharStringUninitializedCopy(get_allocator(), internalLayout().BeginPtr(), internalLayout().EndPtr(), pDetached);
	...
	basic_string<T, Allocator>& basic_string<T, Allocator>::append(const value_type* pBegin, const value_type* pEnd)
	{
		if(pBegin != pEnd)
		{
			...
			if((nOldSize + n) > nCapacity)
			{
				...
				DeallocateSelf();
				internalLayout().SetHeapBeginPtr(get_allocator(), pNewBegin);
	...
	basic_string<T, Allocator>& basic_string<T, Allocator>::append_sprintf_va_list(const value_type* pFormat, va_list arguments)
	{
		...
		nReturnValue = eastl::Vsnprintf<Allocator>(nullptr, 0, pFormat, arguments);
		...
		if (nReturnValue > 0)
		{
			...
			nReturnValue = eastl::Vsnprintf<Allocator>(internalLayout().BeginPtr() + nInitialSize, (size_t)(nReturnValue + 1),
											pFormat, arguments);
		}
		...
		nReturnValue = eastl::Vsnprintf<Allocator>(internalLayout().EndPtr(), (size_t)nInitialRemainingCapacity + 1,
										pFormat, arguments);
		...
		else if(nReturnValue < 0) // If vsnprintf is non-C99-standard
		{
			...
			for(; (nReturnValue < 0) && (n < 1000000); n *= 2)
			{
				...
				nReturnValue = eastl::Vsnprintf<Allocator>(internalLayout().BeginPtr() + nInitialSize, nCapacity + 1, pFormat, arguments);

				if(nReturnValue == (int)(unsigned)nCapacity)
				{
					resize(++n);
					nReturnValue = eastl::Vsnprintf<Allocator>(internalLayout().BeginPtr() + nInitialSize, nCapacity + 2, pFormat, arguments);
				}
			}
		}
	...
	inline void basic_string<T, Allocator>::pop_back()
	{
		...
		{
			auto m = make_modifier(get_allocator(), &internalLayout().EndPtr()[-1], sizeof internalLayout().EndPtr()[-1]);
	...
	basic_string<T, Allocator>& basic_string<T, Allocator>::assign(size_type n, value_type c)
	{
		...
		{
			CharTypeAssignN(get_allocator(), internalLayout().BeginPtr(), n, c);
			erase(internalLayout().BeginPtr() + n, internalLayout().EndPtr());
		}
		else
		{
			CharTypeAssignN(get_allocator(), internalLayout().BeginPtr(), internalLayout().GetSize(), c);
	...
	basic_string<T, Allocator>& basic_string<T, Allocator>::assign(const value_type* pBegin, const value_type* pEnd)
	{
		...
		if(n <= internalLayout().GetSize())
		{
			tr.memmove(internalLayout().BeginPtr(), pBegin, (size_t)n * sizeof(value_type));
			erase(internalLayout().BeginPtr() + n, internalLayout().EndPtr());
		}
		else
		{
			tr.memmove(internalLayout().BeginPtr(), pBegin, (size_t)(internalLayout().GetSize()) * sizeof(value_type));
		...
		if(n) // If there is anything to insert...
		{
			if(internalLayout().GetRemainingCapacity() >= n) // If we have enough capacity...
			{
				...
				if(nElementsAfter >= n) // If there's enough space for the new chars between the insert position and the end...
				{
					...
					CharStringUninitializedCopy(get_allocator(), (internalLayout().EndPtr() - n) + 1, internalLayout().EndPtr() + 1, internalLayout().EndPtr() + 1);
					internalLayout().SetSize(nSavedSize + n);
					mem_wrap(get_allocator()).memmove(const_cast<value_type*>(p) + n, p, (size_t)((nElementsAfter - n) + 1) * sizeof(value_type));
					CharTypeAssignN(get_allocator(), const_cast<value_type*>(p), n, c);
				}
				else
				{
					pointer pOldEnd = internalLayout().EndPtr();
						...
						const size_type nOldSize = internalLayout().GetSize();
					#endif
					CharStringUninitializedFillN(get_allocator(), internalLayout().EndPtr() + 1, n - nElementsAfter - 1, c);
					internalLayout().SetSize(internalLayout().GetSize() + (n - nElementsAfter));
						...
						try
						{
							...
							const size_type nSavedSize = internalLayout().GetSize();
							CharStringUninitializedCopy(get_allocator(), p, pOldEnd + 1, internalLayout().EndPtr());
					...
					CharTypeAssignN(get_allocator(), const_cast<value_type*>(p), nElementsAfter + 1, c);
				}
			}
			else
			{
				...
				iterator pNewEnd = CharStringUninitializedCopy(get_allocator(), internalLayout().BeginPtr(), p, pNewBegin);
				pNewEnd          = CharStringUninitializedFillN(get_allocator(), pNewEnd, n, c);
				pNewEnd          = CharStringUninitializedCopy(get_allocator(), p, internalLayout().EndPtr(), pNewEnd);
				{
					auto mt = make_modifier(get_allocator(), pNewEnd, sizeof *pNewEnd);
					*pNewEnd         = 0;
				}
		...
		if(n)
		{
			...
			if(bCapacityIsSufficient && !bSourceIsFromSelf)
			{
				...
				if(nElementsAfter >= n) // If there are enough characters between insert pos and end
				{
					...
					internalLayout().SetSize(nSavedSize + n);
					auto tr = mem_wrap(get_allocator());
					tr.memmove(const_cast<value_type*>(p) + n, p, (size_t)((nElementsAfter - n) + 1) * sizeof(value_type));
					tr.memmove(const_cast<value_type*>(p), pBegin, (size_t)(n) * sizeof(value_type));
				}
				else
				{
					pointer pOldEnd = internalLayout().EndPtr();
					...
					CharStringUninitializedCopy(get_allocator(), pMid, pEnd, internalLayout().EndPtr() + 1);
					internalLayout().SetSize(internalLayout().GetSize() + (n - nElementsAfter));
					...
					CharStringUninitializedCopy(get_allocator(), pBegin, pMid, const_cast<value_type*>(p));
				}
			}
			else // Else we need to reallocate to implement this.
			{
				...
				pointer pNewEnd = CharStringUninitializedCopy(get_allocator(), internalLayout().BeginPtr(), p, pNewBegin);
				pNewEnd         = CharStringUninitializedCopy(get_allocator(), pBegin, pEnd, pNewEnd);
				pNewEnd         = CharStringUninitializedCopy(get_allocator(), p, internalLayout().EndPtr(), pNewEnd);
				{
					auto mt = make_modifier(get_allocator(), pNewEnd, sizeof *pNewEnd);
	...
	basic_string<T, Allocator>::erase(const_iterator p)
	{
		...
		mem_wrap(get_allocator()).memmove(const_cast<value_type*>(p), p + 1, (size_t)(internalLayout().EndPtr() - p) * sizeof(value_type));
		internalLayout().SetSize(internalLayout().GetSize() - 1);
	...
	basic_string<T, Allocator>::erase(const_iterator pBegin, const_iterator pEnd)
	{
		...
		if(pBegin != pEnd)
		{
			mem_wrap(get_allocator()).memmove(const_cast<value_type*>(pBegin), pEnd, (size_t)((internalLayout().EndPtr() - pEnd) + 1) * sizeof(value_type));
			const size_type n = (size_type)(pEnd - pBegin);
	...
	basic_string<T, Allocator>& basic_string<T, Allocator>::replace(const_iterator pBegin, const_iterator pEnd, size_type n, value_type c)
	{
		...
		if(nLength >= n)
		{
			CharTypeAssignN(get_allocator(), const_cast<value_type*>(pBegin), n, c);
			erase(pBegin + n, pEnd);
		}
		else
		{
			CharTypeAssignN(get_allocator(), const_cast<value_type*>(pBegin), nLength, c);
	...
	basic_string<T, Allocator>& basic_string<T, Allocator>::replace(const_iterator pBegin1, const_iterator pEnd1, const value_type* pBegin2, const value_type* pEnd2)
	{
		...
		if(nLength1 >= nLength2) // If we have a non-expanding operation...
		{
			...
			if((pBegin2 > pEnd1) || (pEnd2 <= pBegin1))  // If we have a non-overlapping operation...
				tr.memcpy(const_cast<value_type*>(pBegin1), pBegin2, (size_t)(pEnd2 - pBegin2) * sizeof(value_type));
			else
				tr.memmove(const_cast<value_type*>(pBegin1), pBegin2, (size_t)(pEnd2 - pBegin2) * sizeof(value_type));
		...
		else // Else we are expanding.
		{
			if((pBegin2 > pEnd1) || (pEnd2 <= pBegin1)) // If we have a non-overlapping operation...
			{
				...
				if((pEnd2 <= pBegin1) || (pBegin2 > pEnd1))
					tr.memcpy(const_cast<value_type*>(pBegin1), pBegin2, (size_t)(pMid2 - pBegin2) * sizeof(value_type));
				else
					tr.memmove(const_cast<value_type*>(pBegin1), pBegin2, (size_t)(pMid2 - pBegin2) * sizeof(value_type));
			...
			else // else we have an overlapping operation.
			{
				...
				pointer pNewEnd = CharStringUninitializedCopy(get_allocator(), internalLayout().BeginPtr(), pBegin1, pNewBegin);
				pNewEnd         = CharStringUninitializedCopy(get_allocator(), pBegin2, pEnd2,   pNewEnd);
				pNewEnd         = CharStringUninitializedCopy(get_allocator(), pEnd1,   internalLayout().EndPtr(),   pNewEnd);
				{
					auto mt = make_modifier(get_allocator(), pNewEnd, sizeof *pNewEnd);
					*pNewEnd        = 0;
				}
	...
	basic_string<T, Allocator>::copy(value_type* p, size_type n, size_type position) const
	{
		...
		const size_type nLength = eastl::min_alt(n, internalLayout().GetSize() - position);
		CharStringUninitializedCopy(get_allocator(), internalLayout().BeginPtr() + position, internalLayout().BeginPtr() + position + nLength, p);
		return nLength;
	}
	...
	void basic_string<T, Allocator>::swap(this_type& x)
	{
		if(get_allocator() == x.get_allocator() || (internalLayout().IsSSO() && x.internalLayout().IsSSO())) // If allocators are equivalent...
		{
			...
			eastl::swap(internalLayout(), x.internalLayout());
			get_allocator().track_post(&internalLayout(), sizeof internalLayout());
			get_allocator().track_post(&x.internalLayout(), sizeof x.internalLayout());
		}
		else // else swap the contents.
		{
			...
			*this = x;                   // itself call this member swap function.
			x     = temp;
			get_allocator().track_post(this, sizeof *this);
			get_allocator().track_post(&x, sizeof x);
		}
	}
	...
	{
		auto r = internalLayout().BeginPtr();
		auto m = make_modifier(get_allocator(), r, (internalLayout().EndPtr() - r) * sizeof *r);
	...
	basic_string<T, Allocator>::InsertInternal(const_iterator p, value_type c)
	{
		...
		{
			const size_type nSavedSize = internalLayout().GetSize();
			mem_wrap(get_allocator()).memmove(const_cast<value_type*>(p) + 1, p, (size_t)(internalLayout().EndPtr() - p) * sizeof(value_type));
			{
				auto m = make_modifier(get_allocator(), internalLayout().EndPtr() + 1, sizeof *(internalLayout().EndPtr() + 1));
				*(internalLayout().EndPtr() + 1) = 0;
			}
			{
				auto m = make_modifier(get_allocator(), pNewPosition, sizeof *pNewPosition);
				*pNewPosition = c;
			}
		...
		else
		{
			...
			pNewPosition = CharStringUninitializedCopy(get_allocator(), internalLayout().BeginPtr(), p, pNewBegin);
			{
				auto m = make_modifier(get_allocator(), pNewPosition, sizeof *pNewPosition);
				*pNewPosition = c;
			}
	...
	void basic_string<T, Allocator>::SizeInitialize(size_type n, value_type c)
	{
		...
		CharStringUninitializedFillN(get_allocator(), internalLayout().BeginPtr(), n, c);
		internalLayout().SetSize(n);
		{
	...
	void basic_string<T, Allocator>::AllocateSelf(size_type n)
	{
		...
		if(n > SSOLayout::SSO_CAPACITY)
		{
			...
			internalLayout().SetHeapBeginPtr(get_allocator(), pBegin);
			internalLayout().SetHeapCapacity(n);
