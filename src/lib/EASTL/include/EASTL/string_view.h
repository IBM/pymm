...
namespace eastl
{
	template <typename T, typename Tracker = DefaultTracker>
	...
	public:
		...
		/* CharStringUninitializedCopy needs a tracker. basic_string_view does not (yet)
		 * contain a Tracker, so we must construct one. But the provided type may have
		 * a protected destructor, a consequence of use as a base type of allocator
		 * (and -Weffc++).
		 */
		struct destructible_tracker
			: public Tracker
		{
		};

		// 21.4.2.6, string operations
		/* memory alteration in string_view. Need a tracker for it. */
		size_type copy(T* pDestination, size_type count, size_type pos = 0) const
		{
			...
			destructible_tracker tr;
			auto* pResult = CharStringUninitializedCopy(tr, mpBegin + pos, mpBegin + pos + count, pDestination);
