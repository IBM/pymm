...
public:
   ...
   *
   */
  Spsc_bounded_lfq(size_t queue_size, void * buffer) :
    _size(queue_size),
    _mask(queue_size - 1),
    _head(0),
    _tail(0)
  {
    if(sizeof(T) < 8)
      throw Constructor_exception("Spsc_bounded_lfq element should be 8-bytes or more");
      
    // make sure it's a power of 2
    if(!((queue_size != 0) && ((queue_size & (~queue_size + 1)) == queue_size)))
      throw Constructor_exception("bad parameters to Spsc_bounded_lfq");
      
    if(!check_aligned(buffer, sizeof(T)))
      throw Constructor_exception("buffer must be aligned to T");
 ...
 * version implements a sleep on the consumer when the queue is empty.
 *
 */
template <typename T>
...
private:
  ...
  static const int RETRY_THRESHOLD = 50000000;
  static const long TIMEOUT_NS = 10000000;  // 10ms

  Spsc_bounded_lfq<T> _queue alignas(sizeof(T));
  bool _exit;
  volatile ConsumerState _consumer_state;
  sem_t _sem;

public:
  Spsc_bounded_lfq_sleeping(size_t queue_size, void * buffer)
    : _queue(queue_size, buffer),
      _exit(false),
   ...
   */
  void *buffer_base() const { return static_cast<void *>(_queue.buffer_base()); }
...
private:
  ...
  Spsc_bounded_lfq<T>          queue_ alignas(8);
  bool                         _exit;
