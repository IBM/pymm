...
template<typename DstT, typename SrcT> DstT safe_downsize(SrcT src) {
  DstT val;
  if(sizeof(DstT) == 4)
    val = src & 0xFFFFFFFFU;
  else if(sizeof(DstT) == 2)
    val = src & 0xFFFFU;
  else if(sizeof(DstT) == 1)
    val = src & 0xFFU;  
  else throw Logic_exception("bad downsize type");
  
  if(val != src)
    throw General_exception("bad downsize conversion");
  return val;
}
#endif
