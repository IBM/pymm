...
static void _cpu_mask_add_core_wrapper(cpu_mask_t &mask,
				       unsigned core_first,
				       unsigned core_last,
				       unsigned mac_cores)
{
  if ( core_last < core_first )
    {
      std::ostringstream e;
      e << "invalid core range specified: [" << core_first << ".." << core_last << ")";
      PERR("%s.", e.str().c_str());
    ...
    {
      std::ostringstream e;
      e << "specified core end [" << core_first << ".." << core_last << ") exceeds physical core count. Valid range is [0.." << mac_cores << ")";
      PERR("%s", e.str().c_str());
      throw std::runtime_error(e.str());
    }
...
namespace common
{
  cpu_mask_t get_cpu_mask_from_string(std::string core_string)
  {
    ...
    cpu_mask_t mask;
    auto hardware_total_cores = std::thread::hardware_concurrency();
